...
module mod_logevents
 ...
 ! Tab:10,11 col - Type , function and subroutine code blocks.
    ...
    ! File major version
    integer(kind=int4),  parameter, public  :: mod_logevents_major = 1_int4
    
    ! File minor version
    integer(kind=int4),  parameter, public :: mod_logevents_minor = 0_int4
    
    ! File micro(patch) version
    integer(kind=int4),  parameter, public :: mod_logevents_micro = 0_int4
    
    ! File full version
    integer(kind=int4),  parameter, public :: mod_logevents_fullver = 1000_int4*mod_logevents_major+100_int4*mod_logevents_minor + &
                                                                        10_int4*mod_logevents_micro
    ...
    type, public :: PerfTimerEvent_t
        ...
        integer(kind=int4)     :: m_line_st,m_line_en
        ...
        integer(kind=int8), allocatable, dimension(:,:) :: m_sctimings
        ...
        !DIR$ ATTRIBUTES ALIGN : 64 :: m_timer_delta
        ...
        real(kind=dp)         :: m_actimingd,m_actimingd, &
        ...
        integer(kind=int4)     :: m_nwcycles
        
        ! Number of code hot-spots
        integer(kind=int4)     :: m_nhotspots
    ...
    type, public :: InvArgEvent_t
        ...
        integer(kind=int4)  :: m_loc_st,m_loc_ed
        ...
        integer(kind=int4)     :: m_invargi32,m_invargi64
    ...
    type, public :: IndexOutBoundsEvent_t
        ...
        integer(kind=int4), dimension(2,31) :: m_indices32
    ...
    type :: FPTrapUndEvent_t
        ...
        real(kind=dp)           :: m_undefval
        
        ! Value of FP-Env status flags
        integer(kind=int4)      :: m_flags
    ...
    subroutine createPerfTimerEvent(event,evname,umsg,fname,             &
          ...
          character(len=*),                             intent(in)    :: evname,umsg,fname, &
                                                                         modname,procname
          integer(kind=int4),                           intent(in)    :: locs,loce,nruns
          real(kind=sp),              dimension(:,:),   intent(in)    :: ctimings
          integer(kind=int8),         dimension(:,:),   intent(in)    :: sctimings
          real(kind=dp),              dimension(:,:),   intent(in)    :: dctimings
          type(T_LARGE_INTEGER_X),    dimension(:,:),   intent(in)    :: qpctimings
          real(kind=dp),              dimension(:,:),   intent(in)    :: timersd
          ...
          real(kind=dp),                                intent(in)    :: actimingsd
          real(kind=dp),                                intent(in)    :: asctimingsd
          real(kind=dp),                                intent(in)    :: adctimingsd
          real(kind=dp),                                intent(in)    :: aqpctimingsd
          character(len=40), dimension(6),              intent(in)    :: timert
          integer(kind=int4),                           intent(in)    :: nwcycles,nhotspots
          real(kind=dp),                                intent(in)    :: maxhotspot,gflops
          logical(kind=int4),                           intent(in)    :: errstat
          ! Locals
          character(len=10)  ::  t
          character(len=8)   ::  d
    ...
    subroutine destroyPerfTimerEevent(event,errstat)
          type(PerfTimerEvent_t),       intent(inout) :: event
          logical(kind=int4),           intent(inout) :: errstat
          ! Exec code
    ...
    subroutine createInvArgEvent(event,evname,umsg,fname, &
          ...
          character(len=*),          intent(in)           :: evname,umsg,fname, &
                                                             modname,procname
          integer(kind=int4),        intent(in)           :: locs,loce,severity
          integer(kind=int4),        intent(in), optional :: invargi32
          integer(kind=int8),        intent(in), optional :: invargi64
    ...
    subroutine createFailAllocEvent(event,evname,umsg,fname, &
          ...
          character(len=*),             intent(in)    :: evname,umsg,fname, &
                                                         modname,procname
          integer(kind=int4),           intent(in)    :: locs,loce,severity,astat
    ...
    subroutine createIndexOutBoundsEvent(event,evname,umsg,fname, &
          ...
          character(len=*),                    intent(in)    :: evname,umsg,fname, &
                                                                modname,procname
          integer(kind=int4),                  intent(in)    :: locs,loce,severity
          integer(kind=int4), dimension(2,31), intent(in)    :: indices
          logical(kind=int4),                  intent(inout) :: errstat
          ! Locals
          character(len=10) :: t
          character(len=8)  :: d
    ...
    subroutine createFailDeallocEvent(event,evname,umsg,fname, &
          ...
          character(len=*),               intent(in)    :: evname,umsg,fname, &
                                                           modname,procname
          integer(kind=int4),             intent(in)    :: locs,loce,severity,dstat
          character(len=*),               intent(in)    :: errmsg
          logical(kind=int4),             intent(inout) :: errstat
          ! Locals
          character(len=10) :: t
    ...
    subroutine createFileIOEvent(event,evname,umsg,fname, &
          ...
          integer(kind=int4),        intent(in)     :: iostat
    ...
    subroutine createDisassocPtrEvent(event,evname,umsg,fname, &
          ...
          character(len=*),               intent(in)    :: evname,umsg,fname, &
                                                           modname,procname
          integer(kind=int4),             intent(in)    :: locs,loce,severity
          logical(kind=int4),             intent(in)    :: status
          logical(kind=int4),             intent(inout) :: errstat
          ! Locals
          character(len=10) :: t
          character(len=8)  :: d
    ...
    subroutine createFPPoleEvent(event,evname,umsg,fname, &
          ...
          character(len=*),         intent(in)    :: procname
          integer(kind=int4),       intent(in)    :: locs,loce,severity
          real(kind=dp),            intent(in)    :: poleval
          logical(kind=int4),       intent(inout) :: errstat
          ! Locals
          character(len=10) :: t
          character(len=8)  :: d
    ...
    subroutine destroyWinErrorEvent(event,errstate)
           type(WinError_t),      intent(inout) :: event
           logical(kind=int4),    intent(inout) :: errstate
           ! Exec code ....
           if(errstate) errstate = .false.
           if(event%m_isbuilt == .false.) then
                  errstate = .true.
                  write(ERROR_UNIT,*) "================================================="
                  write(ERROR_UNIT,*) "  destroyWinErrorEvent: [FATAL-ERROR]           "
                  write(ERROR_UNIT,*) "  Attempted destruction of uninitialized object!!"
                  write(ERROR_UNIT,*) "================================================="
                  return 
           end if
              ...
              event%m_winerr      = -1
              event%m_isbuilt     = .false.
    end subroutine destroyWinErrorEvent
