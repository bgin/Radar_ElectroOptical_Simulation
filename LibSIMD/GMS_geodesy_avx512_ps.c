...
#include <math.h>
#include "GMS_geodesy_avx512_ps.h"

#if !defined(VEC16_REM)
...
#define MAX(a,b)  (((a) > (b)) ? (a) : (b))
                    ...
                    void
                    cart_to_geodetic_zmm16r4( const __m512 pos_x,
                                             const __m512 pos_y,
			                     const __m512 pos_z,
			                     const __m512 a,
			                     const __m512 b,
			                     __m512 * __restrict alt,
			                     __m512 * __restrict lon,
			                     __m512 * __restrict lat) {
...
#pragma loop_count min(1),avg(8),max(16)
					  ...
					  float t0 = 1.0+c;
					  float s  = powf(t0+sqrtf(c*c+2.0*c),0.3333333333333333333333333333333F);
					  ...
					  float t6 = q*(1.0F+q);
					  float t7 = 0.5*pp*rr;
					  float r0 = t3+sqrtf(MAX(0.0,t4-(t5/t6)-t7));
					  const float t8 = (r-e2*r0)*(r-e2*r0);
					  float u  = sqrtf(t0+zz);
					  float v  = sqrtf(t0+(1.0F-e2)*zz);
					  const float t9 = a*v;
					  float z0 = b*b*z/t9;
...
#pragma loop_count min(1),avg(8),max(16)
					  ...
					  float c  = e2*e2*ff*rr/(g*g*g);
					  float t0 = 1.0+c;
					  float s  = powf(t0+std::sqrtf(c*c+2.0*c),0.3333333333333333333333333333333F);
					  float t1 = (s+1.0F/s+1.0F)*(s+1.0F/s+1.0F);
					  float pp = ff/(3.0F*t1*g*g);
					  float t2 = e2*e2*pp;
					  float q  = sqrtf(1.0F+2.0F*t2);
					  float t3 = -pp*e2*r/(1.0F+q);
					  float t4 = 0.5*aa*(1.0F+1.0F/q);
					  float t5 = pp*(1.0F-e2)*zz;
					  float t6 = q*(1.0F+q);
			...
			void geodetic_to_cart_zmm16r4(const __m512 a,
						     ...
						     const __m512 lon,
						     const __m512 alt,
						     __m512 * __restrict pos_x,
						     __m512 * __restrict pos_y,
						     __m512 * __restrict pos_z) {
...
#pragma loop_count min(1),avg(8),max(16)
					...
					register const  float s0 = sinf(glat);
					register const  float glon = lon[i];
					register const  float s1 = cosf(glat);
					register float ee  = 1.0F-(b*b)/(a*a);
					register const float s3 = sinf(glon);
					register float om2 = 1.0F-ee;
					register float q  = std::sqrtf(1.0-ee*s0*s0);
					register const float salt = alt[i];
					register const float s4 = cosf(glon);
					register const float s2 = tanf(glat);
					register float d = sqrtf(1.0F-ee*s2*s2);
...
#pragma loop_count min(1),avg(8),max(16)
					...
					register const float s3 = sinf(glon);
					register float om2 = 1.0F-ee;
					register float q  = sqrtf(1.0-ee*s0*s0);
					register const float salt = alt[i];
					register const float s4 = cosf(glon);
					register const float s2 = tanf(glat);
					register float d = sqrtf(1.0F-ee*s2*s2);
