...
!
integer(int4),parameter :: i_print=0  ! (0/1/2)  Test output printing off/on
!                               ! Output channel defined by i_out
!
integer(int4),parameter :: i_out=6    ! Output channel to screen
...
!
integer(int4) :: i_log        ! (0/1)      Logging off/on
integer(int4) :: i_prt        ! (0/1)      Printing off/on
integer(int4) :: i_tst        ! (0,1,2...) Test level off/on
!
!
! Standard unit numbers of input & output files
!
integer(int4) :: lu_err  ! standard error file
integer(int4) :: lu_inp  ! standard input file
integer(int4) :: lu_log  ! standard logging
integer(int4) :: lu_prt  ! standard print output
integer(int4) :: lu_tst  ! standard test output
!
...
integer(int4),    intent(in)     :: iufind    ! Indicator for search of unit number
integer(int4),   intent(inout)   :: iunit     ! Unit number
integer(int4),   intent(out)     :: iostat    ! Error indicator
!
!  4. Subroutines used
!
!     Z_FLUNIT
...
integer(int4) :: junit               ! temporary unit number
logical(int4) :: lexist              ! indicator if a file exists
logical(int4) :: lopen               ! indicator if a file is opened
integer(int4) :: iuerr               ! error indicator from Z_FLUNIT
integer(int4) :: ilpos               ! start position for appending node number
!-------------------------------------------------------------------------------------
! initialisations
!-------------------------------------------------------------------------------------
iostat = 0
...
!Type     I/O           Name         Description
!----------------------------------------------------------
integer(int4), intent(out) :: iunit       ! resulting unit number
integer(int4), intent(out) :: ierr        ! error level
!
!  4. Subroutines used
!
!     None
...
! local parameters
!
integer(int4) :: junit                       ! counter for unit numbers
logical(int4) :: lopen                       ! indicator if a unit number is connected to a file
logical(int4) :: lnot                        ! indicates if a forbidden unit number is checked
integer(int4) :: i_not                       ! counter to check forbidded unit numbers
!
!---------------------------------------------------------------------------------
!  range of unit numbers to search
!
integer(int4), parameter :: lu_min=60     ! minimum unit number
integer(int4), parameter :: lu_max=200    ! maximum unit number
!
! specification of forbidden unit numbers
!
integer(int4), parameter :: lu_nr=3   ! number of forbidden unit numbers
integer(int4) :: lu_not(lu_nr)           ! list of forbidden unit numbers
!----------------------------------------------------------------------------------
lu_not(1) = 100
lu_not(2) = 101
lu_not(3) = 102
