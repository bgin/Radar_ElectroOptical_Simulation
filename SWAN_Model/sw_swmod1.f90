...
!
      INTEGER(int4) ::          ELINT, KARNR, LENCST
      real(dp)      :: ELREAL
      LOGICAL(int4) ::          CHGVAL
!
...
!
      INTEGER(int4) ::      MXQ, MYQ
      REAL(sp)      ::      DXQ, DYQ, VERNUM
!
!  8. Subroutines and functions used
!
!     ---
...
!
      INTEGER(int4) :: EXPORT, FUNHI , FUNLO , HIOPEN
      INTEGER(int4) :: IMPORT, INPUTF, ITMOPT, IUNMAX
      INTEGER(int4) :: IUNMIN, PRINTF, PRTEST, SCREEN
...
!
      INTEGER(int4) :: ITEST, ITRACE, LEVERR, MAXERR
      LOGICAL(int4) :: LTRACE
!
!  8. Subroutines and functions used
!
...
!
      INTEGER(int4) :: NMOVAR, MOUTPA
      PARAMETER (NMOVAR = 59, MOUTPA=5)                             
!
!  7. Local variables
!
...
!
      INTEGER(int4) :: ERRPTS
      INTEGER(int4) :: INRHOG,         IUBOTR
      INTEGER(int4) ::  OVSVTY(NMOVAR), SPCPOW
      REAL(sp) ::    AKPOWR,         ALCQ,        ALPQ,          COSCQ
      REAL(sp) ::    COSPQ,          DXK,         DYK
      REAL(sp) ::    OVEXCV(NMOVAR),              OVLEXP(NMOVAR)
      REAL(sp) ::    OVLLIM(NMOVAR),              OVHEXP(NMOVAR)
      REAL(sp) ::    OVULIM(NMOVAR),              SINCQ,         SINPQ
      REAL(sp) ::   XPQ,            XQLEN,       XQP,           YPQ
      REAL(sp) ::    YQLEN,          YQP,         OUTPAR(MOUTPA)
!
!  8. Subroutines and functions used
!
!     ---
...
!
      INTEGER(int4) :: ICOND
      INTEGER(int4) :: IGTYPE(NUMGRD),           LEDS(NUMGRD)
      INTEGER(int4) :: MXG(NUMGRD), MYG(NUMGRD), NWAMN
      INTEGER(int4) :: OPTG,        NBFILS,      NBSPEC,      NBGRPT
      REAL(sp) ::    ALPG(NUMGRD),             COSPG(NUMGRD)
      REAL(sp) ::    COSVC,       COSWC,       DXG(NUMGRD)
      REAL(sp) ::    DYG(NUMGRD), EXCFLD(NUMGRD)
      REAL(sp) ::    RDTIM,       SINPG(NUMGRD)
      REAL(sp) ::    SINVC,       SINWC,       STAGX(NUMGRD)
      REAL(sp) ::    STAGY(NUMGRD),            XOFFS
      REAL(sp) ::    XPG(NUMGRD), YOFFS,       YPG(NUMGRD)
      LOGICAL(int4) :: CVLEFT,      LXOFFS,      VARFR
      LOGICAL(int4) :: VARWI,       VARWLV,      DYNDEP
      LOGICAL(int4) :: VARAST
!
...
!
      INTEGER(int4) ::      IFLDYN(NUMGRD)
      INTEGER(int4) ::      IFLIDL(NUMGRD), IFLIFM(NUMGRD), IFLNHF(NUMGRD),    &
                            IFLNHD(NUMGRD), IFLNDS(NUMGRD), IFLNDF(NUMGRD)
      REAL(sp) ::           IFLBEG(NUMGRD), IFLINT(NUMGRD), IFLEND(NUMGRD),    &
                            IFLTIM(NUMGRD)
      REAL(sp) ::           IFLFAC(NUMGRD)
...
!
      INTEGER(int4) ::             MBOT,        MNUMS,       MQUAD
      INTEGER(int4) ::             MSETUP,      MSHAPE,      MSPPAR,      MSURF
      INTEGER(int4) ::             MTRIAD,      MWCAP,       MWIND
      INTEGER(int4) ::             MICMAX
      INTEGER(int4) ::             MDIFFR                                         
      INTEGER(int4) ::             MDISP                                          
!
...
!
      INTEGER(int4) ::              JABIN,       JABLK,       JAOLD
      INTEGER(int4) ::              JASTD2,      JASTD3
      INTEGER(int4) ::              JCDRAG,      JDHS,        JDIS0
      INTEGER(int4) ::              JDIS1,       JDISS,       JDPSAV,      JDP1
      INTEGER(int4) ::              JDP2
      INTEGER(int4) ::              JDP3,        JDTM,        JFRC2,       JFRC3
      INTEGER(int4) ::              JDSXB
      INTEGER(int4) ::              JDSXS
      INTEGER(int4) ::              JDSXW
      INTEGER(int4) ::              JHS,         JHSIBC,      JLEAK
      INTEGER(int4) ::              JLEK1,       JMAT5,       JMAT6,       JMATD
      INTEGER(int4) ::              JMATL,       JMATR,       JMATU
      INTEGER(int4) ::              JP4D,        JP4S,        JPBTFR,      JPTRI
      INTEGER(int4) ::              JPWBRK,      JPWCAP,      JPWNDA,      JPWNDB
      INTEGER(int4) ::              JQB,         JSETUP,      JSTP,        JTAUW
      INTEGER(int4) ::              JUBOT,       JUSTAR,      JVX1,        JVX2
      INTEGER(int4) ::              JVX3,        JVY1,        JVY2,        JVY3
      INTEGER(int4) ::              JWLV1,       JWLV2,       JWLV3
      INTEGER(int4) ::              JWX2,        JWX3
      INTEGER(int4) ::              JWY2,        JWY3,        JZEL  ,      JPBOT
      INTEGER(int4) ::              MCMVAR,                   MTSVAR,      JURSEL
      INTEGER(int4) ::              JBOTLV,      MSWMATR,     MLSWMAT
!
...
!
      INTEGER(int4) ::              IXCGRD(MICMAX), IYCGRD(MICMAX), KCGRD(MICMAX)
      INTEGER(int4) ::              ICOMP,       MCGRD
      INTEGER(int4) ::              MDC,         MDC4MA,      MDC4MI,      MMCGR
      INTEGER(int4) ::              MSC,         MSC4MA,      MSC4MI,      MTC
      INTEGER(int4) ::              MXC,         MYC,         NX,          NY
      INTEGER(int4) ::              NGRBND
      REAL(sp) ::                ALCP,        ALPC,        COSPC,       DDIR
      REAL(sp) ::                DX,          DXRP,        DY,          DYRP
      REAL(sp) ::                FRINTF,      FRINTH,      SHIG,        SINPC
      REAL(sp) ::                SLOW,        SPDIR1,      SPDIR2,      XCLEN
      REAL(sp) ::                XCP,         XPC,         YCLEN,       YCP
      REAL(sp) ::                YPC
      REAL(sp) ::                XCGMIN,      XCGMAX,      YCGMIN,      YCGMAX
      LOGICAL(int4) ::             FULCIR
...
!
      REAL(sp) :: CASTD
      REAL(sp) :: DEGRAD,      DNORTH,      GRAV,        PI
      REAL(sp) :: PI2,         PWTAIL(10),  RHO,         WLEV
!
...
!
      INTEGER(int4) ::              DSHAPE,      FSHAPE
      INTEGER(int4) ::              ICMAX
      INTEGER(int4) ::              IBOT,        ICOR,        ICUR
      INTEGER(int4) ::              IDBR,        IDIF,        IGEN,        IINC
      INTEGER(int4) ::              IPRE,        IQUAD,       IREFR,       ISURF
      INTEGER(int4) ::              ITERMX,      ITFRE,       ITRIAD
      INTEGER(int4) ::              IWCAP,       IWIND,       LSETUP
      INTEGER(int4) ::              MXITST,      MXITNS,                   NCOR
      INTEGER(int4) ::              NSTATC,      NSTATM,      NUMOBS,      NCOMPT   
      INTEGER(int4) ::              IDIFFR                                         
      REAL(sp) ::                DEPMIN,      PBOT(MBOT),  PNUMS(MNUMS)
      REAL(sp) ::                PSETUP(MSETUP),           PSHAPE(MSHAPE)
      REAL(sp) ::                PSURF(MSURF),             PTRIAD(MTRIAD)
      REAL(sp) ::                PWCAP(MWCAP),             PWIND(MWIND)
      REAL(sp) ::                SIGMAG
      REAL(sp) ::                SPPARM(MSPPAR),SY0,       U10
      REAL(sp) ::                WDIC,          WDIP,      HSRERR
      REAL(sp) ::                PQUAD(MQUAD)
!DIR$ ATTRIBUTES ALIGN : 64 :: RCOMPT
      REAL(sp) ::                RCOMPT(50,5)
      REAL(sp) ::                PDIFFR(MDIFFR)                                 
      LOGICAL(int4) ::             ACUPDA
      LOGICAL(int4) ::             BNDCHK,      BNAUT,       ONED,        BRESCL
      LOGICAL(int4) ::             CSETUP
...
!
      INTEGER(int4) :: ICOTES,      INTES,       IOUTES
      INTEGER(int4) :: IPTST
      INTEGER(int4) :: IFPAR,       IFS1D,       IFS2D
      INTEGER(int4) :: LXDMP,       LYDMP,       MAXMES,      NEGMES
      INTEGER(int4) :: NPTST,       NPTSTA
      REAL(sp) ::    UNDFLW
      LOGICAL(int4) :: TESTFL
...
!                 in subroutine SANDL)
!
      INTEGER(int4) :: PROPSC,    KSPHER,    KREPTX
      INTEGER(int4) :: PROPSL
      INTEGER(int4) :: PROPSS,    PROPSN,    PROJ_METHOD
      INTEGER(int4) :: PROPFL
      REAL(sp) ::    WAVAGE,    REARTH,    LENDEG
      REAL(sp) ::    COSLAT(10)
...
!
      INTEGER(int4) ::  IFACMX, IFACMY
      REAL(sp) ::     BEGBOU, TIMERB, TINTBO
!TIMG!
