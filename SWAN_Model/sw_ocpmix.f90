...
!           (6): second
!
      INTEGER(int4), dimension(6) :: INTTIM                                                 ! 30.74
!
...
!     II     : ??
!
      INTEGER(int4), dimension(12) :: IDYMON 
      INTEGER(int4) :: IYEAR, IYRM1, IDNOW, I, II
!
!     LEAPYR : Whether year in INTTIM(1) is a leapyear
!     LOGREF : ??
!
      LOGICAL(int4) :: LEAPYR, LOGREF
!
...
!
      IYEAR = INTTIM(1)
      IYRM1 = IYEAR-1
      LEAPYR=(MOD(IYEAR,4).EQ.0.AND.MOD(IYEAR,100).NE.0).OR. MOD(IYEAR,1000).EQ.0
  ...
  10    CONTINUE
      ...
      ENDIF
      DTTIME = DTTIME + 3600.*REAL(INTTIM(4)) + 60.*REAL(INTTIM(5)) +   REAL(INTTIM(6))
...
!           (6): second
!
      INTEGER(int4), dimension(6) :: INTTIM
!
...
!
      INTEGER(int4) :: IDAYYR, IDYCEN, IDYMIL, IFOUR
!
...
!
      INTEGER(int4) :: I4, ICEN, IDYMN,  IDYNOW, IMIL, IMN, IYR, IYEAR,NDAY, NOWDAY 
      INTEGER(int4), dimension(12) :: IDYMON
!
...
!  6. LOCAL VARIABLESC
!
      INTEGER(int4), dimension(6) ::     ITIME
!
!  8. SUBROUTINE USED
!
...
!*****************************************************************
!                                                                *
      SUBROUTINE REPARM (NDSL, NDSD, IDLA, IDFM, RFORM, NHEDF, IDYN, NHEDT, LOGC, NHEDC)                   !40.00
...
!                       or vector component
!
      INTEGER(int4) ::   IDFM, IDLA,  NDSL, NDSD, NHEDF, NHEDT, NHEDC, IDYN
!
...
!
      INTEGER(int4) ::   IENT, IH, IOSTAT
!
!     HEDLIN : Content of a header line
!     KEYWIS : ??
!
      LOGICAL(int4) ::   KEYWIS, BNEW
!
!     OLDFIL : ??
!
      CHARACTER(len=80) :: HEDLIN
      CHARACTER(len=36) :: OLDFIL 
!                                                                         30.82
!  8. SUBROUTINE USED
!
      LOGICAL(int4) ::   STPNOW                                                   ! 34.01
!
!  9. SUBROUTINES CALLING
!
! 10. ERROR MESSAGES
...
!                       before each array
!
      INTEGER(int4) ::   IDFM, IDLA, MXA, MYA, NDSD, NDSL, NHED, NHEDF
!
...
!     NUMFIL : ??
!
      INTEGER(int4) ::   IERR, IENT, IOERR, IH, IX, IY, NUMFIL                    ! 40.02
!
!     HEDLIN : Content of a header line
!
      CHARACTER(len=80) :: HEDLIN 
...
!$    ELSE                                                                40.31
         ...
         IF ( SCREEN.NE.PRINTF .AND. INODE.EQ.MASTER )  WRITE (SCREEN, 10) SUBNAM                   ! 40.30
...
!
      INTEGER(int4), SAVE :: IERR=0, IERRF=0                                    !40.03
      INTEGER(int4) :: ILPOS                                                      ! 40.30
!
...
!
      INTEGER(int4) ::  IESUCC, IENUNF, IEUNBD, IENODD,   &
...
!
      INTEGER(int4) ::   IENT, IFO, IFUN, II, IOSTTM, IS, IUTTM
!
!     EXIST     if true, file exists
!     OPENED    if true, file is opened
!
      LOGICAL(int4) ::   EXIST, OPENED
!
!     S
!     F
!     FILTTM   auxiliary
...
!           Was the file opened already ?
           ...
           OPENED=OPENED, NUMBER=IUTTM)
            IF (IOSTTM .NE. IESUCC) THEN
               IF (IOSTAT.GT.-1) CALL MSGERR (2,'Inquire failed, filename: '//DDNAME_L) 
...
!           If file does not exist, print term. error
            ...
            END IF
            IF (OPENED) THEN
               IF (IOSTAT.GT.-1)    CALL MSGERR (2, 'File is already opened: '//DDNAME_L) 
...
!           Assign free unit number
            ...
            ENDIF
         END IF
         OPEN (UNIT=IUNIT,ERR=999,IOSTAT=IOSTTM,FILE=DDNAME, STATUS=FISTAT(IS),ACCESS='SEQUENTIAL',FORM=FORM(IFO))         

               
      ELSE
         INQUIRE (UNIT=IUNIT, NAME=FILTTM, IOSTAT=IOSTTM,  EXIST=EXIST, OPENED=OPENED)
               
         IF (IOSTTM .NE. IESUCC) THEN
            IF (IOSTAT.GT.-1) CALL MSGERR (2,'Inquire failed, filename: '//FILTTM)
         ...
         IF (OPENED) THEN
            ...
            IF (FILTTM.NE.DDNAME .AND. FILTTM.NE.EMPTY) THEN
              IF (IOSTAT.GT.-2) THEN
                WRITE (PRINTF, '(A, I4, 6A)') ' unit', IUNIT,   ' filenames: ', FILTTM, ' and: ', DDNAME
...
!             close old file and open new one with given filename
              CLOSE (IUNIT)
              OPEN (UNIT=IUNIT,ERR=999,IOSTAT=IOSTTM,STATUS=FISTAT(IS), FILE=DDNAME,ACCESS='SEQUENTIAL',FORM=FORM(IFO))
         ...
         ELSE
            OPEN (UNIT=IUNIT,ERR=999,IOSTAT=IOSTTM,STATUS=FISTAT(IS),ACCESS='SEQUENTIAL',FORM=FORM(IFO))
...
!
      INTEGER(int4), INTENT(IN) :: NUMITM                                      !  40.03
!
...
!              in quotes, 1: other
!
      INTEGER(int4) ::   ICR1, IENT, ILL, IITM, LENLIN, RITM
!
...
!                 empty item
            ...
            ENDIF
         ENDIF
         IF (ITEST.GE.250) WRITE (PRTEST, 165) CRL, RITM,IITM, ICR1
...
!
      INTEGER(int4), dimension(*) ::   IARRAY 
      INTEGER(int4) :: LENARR, IENT, IERR                              ! 30.72
                                                           ! 30.81
!
...
!
      INTEGER(int4) ::   OPMLFC, OPMNLI                                            !30.72
!
...
!
      INTEGER(int4) ::   IC, II, LENS1, LENS2, LL, MC1, MCHAR, MM, NSL
!
...
!
      ...
      IF (LENARR.GT.360) THEN
        ...
        WRITE (PRTEST, *) ' test COPYCH  ', MOVE, LENARR, LENS1, ' ', STRING(1:80) 
        ...
        IF (LENS1.GT.LENS2) THEN
          DO 70 II = LENS1+1, LENS2
            IF (STRING(II:II) .NE. ' ') THEN
              ...
              IF (ITEST.GE.50) WRITE (PRTEST, *) ' test COPYCH  ',   MOVE, LENARR, LENS1, LENS2, ' ', STRING(1:80)
...
!       first the string is filled with blanks
        ...
        DO 160 II = 1, LENARR
          ...
          DO 140 IC = 1, OPMNLI
            ...
            ELSE
              IF (CHAR(LL) .NE. ' ') THEN
                IF (IERR.NE.802)   CALL MSGERR(1, 'string shorter than capacity of array')                                        !  30.81
            
                IF (ITEST.GE.50) WRITE (PRTEST, *) ' test COPYCH  ',   MOVE, LENARR, LENS1, LENS2, ' ', STRING
...
!           IF (ITEST.GE.250) WRITE (PRTEST, *) NSL, LL, STRING(NSL:NSL)
 ...
 160    CONTINUE
      ...
      ENDIF
 165  IF (ITEST.GE.230) WRITE (PRTEST, 167) LENS1, STRING, MOVE,(IARRAY(II), II=1,LENARR)
