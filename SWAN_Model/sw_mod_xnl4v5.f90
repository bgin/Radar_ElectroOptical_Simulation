...
!
integer(int4) :: iufind  ! Specifies handling of unit numbers, see Z_FILEIO
integer(int4) :: iscreen ! identifier for screen, set in XNL_INIT
!
!  unit numbers for I/O
!
integer(int4) :: luq_bqf  ! binary file storing and retrieving precomputed loci
integer(int4) :: luq_cfg  ! user defined configuration
integer(int4) :: luq_err  ! file with error messages
integer(int4) :: luq_fil  ! test output for filtering
integer(int4) :: luq_grd  ! ASCII file storing and retrieving precomputed loci
integer(int4) :: luq_int  ! test file for test output of integration
integer(int4) :: luq_loc  ! statistics about computed loci
integer(int4) :: luq_log  ! logging
integer(int4) :: luq_prt  ! general print file for quadruplets
integer(int4) :: luq_trf  ! testing transformation of loci
integer(int4) :: luq_tst  ! test file for quadruplets
integer(int4) :: luq_txt  ! reading (error) text file
integer(int4) :: luq_t13  ! test of basis integration
!------------------------------------------------------------------------------
!  physical coefficients, to be obtained through interface XNL_INIT
!------------------------------------------------------------------------------
real(sp) :: q_grav     ! gravitational acceleration (Earth = 9.81 m/s^2)
real(sp) :: qf_tail    ! power of spectral tail of E(f), e.g. -4,, -4.5, -5
!               ! these values must be set in the interface routine
!------------------------------------------------------------------------------
!  filtering coefficients
!------------------------------------------------------------------------------
real(sp) :: qf_krat     ! maximum ratio of the interacting wave numbers k1 and k3
real(sp) :: qf_dmax     ! maximum directional difference between k1 and k3
real(sp) :: qf_frac     ! fraction of maximum action density to filter
!
!  program switches, optionally to be reset in routine Q_SETCONFIG
!
integer(int4) :: iq_compact ! switch to compact data
!                 == 0, do not compact
!                 == 1, compact data by elimiting zero contribution along locus
!
integer(int4) :: iq_cple   ! type of coupling coefficient
...
!
integer(int4) :: iq_disp   ! type of dispersion relation, viz. depth dependency
!                 == 1, deep water, possibly with geometric scaling
!                 == 2, linear dispersion relation, w^2 = g.k.tanh(kd)
!                 == 3, nonlinear dispersion relation
!
integer(int4) :: iq_dscale !  switch to activate depth scaling according to
                  !  Herterich and Hasselmann
!                 !  == 0, No depth scaling
!                 !  == 1, depth scaling activated
!
integer(int4) :: iq_filt   !  switch to activate filtering in wave number space
!                 !  ==0, no filtering
!                 !  ==1, filtering activated
!
integer(int4) :: iq_gauleg ! switch for Gauss-Legendre interpolation
!                 !  == 0, No Gauss-Legendre, default
!                 !  > 0   Gauss-Legendre, iq_gauleg is number of points
!
integer(Int4) :: iq_geom   !  type of scaling
...
!
integer(int4) :: iq_log    !  switch to activate logging to file QBASE//.LOG
!                 !  == 0, No print output
!                 !  == 1, print output
!
integer(int4) :: iq_lump   !  switch to activate lumping on locus
!                 !  == 0, No lumping
!                 !  == 1, Lumping along locus
!
integer(int4) :: iq_make   !  option to make quadruplet grid
!                    == 1, make when needed (default)
!                    == 2, always make quadruplet grid
!                    == 3, only make grid file
!
integer(int4) :: iq_mod    !  option to redistribute points on locus
!                 !  == 0, Points will be used as computed by tracing algortihm
!                 !  == 1, Equi-distant spacing on points along locus (NLOC1)
!
integer(int4) :: iq_prt    !  switch to activate print output, to file QBASE//.PRT
!                 !  == 0, No print output
!                 !  == 1, print output
!
integer(int4) :: iq_qrule  !  Rule for quadrature method
!                 !  == 1, Trapezoid rule
!                 !  == 2, Simpson rule
!                 !  == 3, Gauss-Legendre rule (in combination with iq_gauleg)
!
integer(int4) :: iq_search ! switch to determine search for a proper grid
!                 == 0, no search is carried out
!                 == 1, search nearest (relative) interaction grid
!
integer(int4) :: iq_screen ! option to send output to the screen
!                 !  == 0, no output is send to screen
!                 !  == 1, output is send to screen
!
integer(int4) :: iq_nsimp  ! switch for Simpson quadrature (must be even)
!                 !  == 0, Default trapezoid rule
!                 !  > 0   N-point Simpson rule
!
integer(int4) :: iq_sym    ! switch to activate use of symmetry reduction
!                 !  == 0, no symmetries are used
!                 !  == 1, symmetry activated (default)
!
integer(int4) :: iq_tail   ! add parametric tail to transfer rate and diagnonal term
!                 !  == 0, no tail is added
!                 !  == 1, parametric tail is added
!
integer(int4) :: iq_test   !  test level, output is directed to unit luqtst
!                 !  == 0, no test output
!                 !  == 1, output of basic I/O
!                 !  == 2, extensive test output
!
integer(int4) :: iq_trace  !  trace option
!                 !  == 0, no trace of subroutine calls
!                 !  > 0,  maximum number of traces per subroutine
!                 !  < 0,  as for >0 but now output is send to the screen
!
integer(int4) :: iq_trf    !  option to print transformed loci to special output file
!                 !  == 0, no output to data file unit luqtrf
!                 !  == 1, test output from routine Q_GETLOCUS
!
integer(int4) :: iq_t13    ! option to output T13 integration
!                 !  ==0, no output
!                 !  ==1, test output of T13 per locus
!
integer(int4) :: iq_xdia    ! switch to activate output to extended DIA data file
!                    == 0, no output
...
!
integer(int4) :: nkq     ! number of wave numbers of quad-grid
integer(int4) :: naq     ! number of angles of quad-grad
integer(int4) :: ncirc   ! number of angles on a full circle
!
integer(int4) :: ia_k1,ik_k1 ! indices of main loop variables
integer(int4) :: ia_k3,ik_k3 ! indices of main loop variables
!
real(sp) :: fqmin      ! lowest frequency in Hz
real(sp) :: fqmax      ! highest frequency in Hz
real(sp) :: q_sector   ! half plane width in degrees (for iq_grid=1,2)
real(sp) :: q_dstep    ! step size for generating BQF files
!
integer(int4), parameter :: mq_stack=10 ! maximum number of elements in stack
!
integer(int4) :: mlocus   ! maximum number of points on locus for defining arrays
integer(int4) :: nlocus0  ! preferred number of points on locus
integer(int4) :: nlocus1  ! number of points on locus as computed in Q_CMPLOCUS
integer(int4) :: klocus   ! number of points on locus as stored in quadruplet database
                 ! based on nlocus0, iq_gauleg and iq_lump (without compacting)
                 ! used in Q_ALLOCATE to define size of data arrays
integer(int4) :: nlocus   ! number of points on locus, equal to klocus
integer(int4) :: nlocusx  ! number of points on locus for use in computation (nlocusx <= nlocus)
!
real(sp) :: kqmin       ! lowest wave number
real(sp) :: kqmax       ! highest wave number
real(sp) :: wk_max      ! maximum weight for wave number interpolation, set in Q_INIT
!
real(sp) :: k0x,k0y,dk0 ! components of initial wave number of locus,
real(sp) :: krefx,krefy ! components of reference wave number for quad-grid
real(sp) :: k1x,k1y     ! components of k1 wave number
real(sp) :: k2x,k2y     ! components of k2 wave number
real(sp) :: k3x,k3y     ! components of k3 wave number
real(sp) :: k4x,k4y     ! components of k4 wave number
real(sp) :: px,py       ! components of difference k1-k3 wave number
real(sp) :: pmag        ! magnitude of P-vector
real(sp) :: pang        ! angle related of P-vector, Pang = atan2(py,px), (radians)
real(sp) :: sang        ! angle of symmytry axis of locus, SANG = PANG +/ pi° (radians)
real(sp) :: xang        ! angle of locus for the case that w1=w3, Xang=atan2(-px,py), (radians)
real(sp) :: q           ! difference of radian frequencies, used in Resio-Tracy method
real(sp) :: kmin_loc    ! minimum wave number of locus along symmetry axis
real(sp) :: kmax_loc    ! maximum wave number of locus along symmetry axis
real(sp) :: kmid        ! wave number at midpoint of locus along symmetry axis
real(sp) :: kmidx       ! x-component of wave number at midpoint of locus along symmetry axis
real(sp) :: kmidy       ! y-component of wave number at midpoint of locus along symmetry axis
real(sp) :: loc_crf     ! circumference of locus in (kx,ky)-space
real(sp) :: loc_area    ! area of locus, measured in (kx-ky)- space
real(sp) :: loc_xz      ! x-coordinate of center of gravity of locus in (kx,ky)-space
real(sp) :: loc_yz      ! y-coordinate of center of gravity of locus in (kx,ky)-space
!
...
!
!
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_nloc
integer(int4), allocatable :: quad_nloc(:,:)   ! number of points on locus
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_ik2
integer(int4), allocatable :: quad_ik2(:,:,:)  ! lower wave number index of k2
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_ia2
integer(int4), allocatable :: quad_ia2(:,:,:)  ! lower direction index of k2
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_ik4
integer(int4), allocatable :: quad_ik4(:,:,:)  ! lower wave number index of k4
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_ia4
integer(int4), allocatable :: quad_ia4(:,:,:)  ! lower direction index of k4
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w1k2
real(sp), allocatable :: quad_w1k2(:,:,:)    ! weight 1 of k2
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w2k2
real(sp), allocatable :: quad_w2k2(:,:,:)    ! weight 2 of k2
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w3k2
real(sp), allocatable :: quad_w3k2(:,:,:)    ! weight 3 of k2
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w4k2
real(sp), allocatable :: quad_w4k2(:,:,:)    ! weight 4 of k2
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w1k4
real(sp), allocatable :: quad_w1k4(:,:,:)    ! weight 1 of k4
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w2k4
real(sp), allocatable :: quad_w2k4(:,:,:)    ! weight 2 of k4
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w3k4
real(sp), allocatable :: quad_w3k4(:,:,:)    ! weight 3 of k4
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_w4k4
real(sp), allocatable :: quad_w4k4(:,:,:)    ! weight 4 of k4
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_zz
real(sp), allocatable :: quad_zz  (:,:,:)    ! compound product of cple*ds*sym/jac
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_t2
real(sp), allocatable :: quad_t2(:,:,:)      ! tail factors for k2 wave number
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_t4
real(sp), allocatable :: quad_t4(:,:,:)      ! tail factors for k4 wave number
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_sym
real(sp), allocatable :: quad_sym(:,:,:)     ! symmetry factor btween k1-k3 and k1-k4
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_jac
real(sp), allocatable :: quad_jac(:,:,:)     ! jacobian term
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_cple
real(sp), allocatable :: quad_cple(:,:,:)    ! coupling coefficient
!DIR$ ATTRIBUTES ALIGN : 64 :: quad_ws
real(sp), allocatable :: quad_ws (:,:,:)     ! (weighted) step size
!
!  characteristic of computed locus
!

!DIR$ ATTRIBUTES ALIGN : 64 :: x2_loc
real(sp), allocatable :: x2_loc(:)   ! k2x coordinates around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: y2_loc
real(sp), allocatable :: y2_loc(:)   ! k2y coordinates around locus
!DIR$ ATTRIBUTES ALIGN : 64 ::  z_loc
real(sp), allocatable :: z_loc(:)    ! data value around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: s_loc
real(sp), allocatable :: s_loc(:)    ! coordinate along locus
!DIR$ ATTRIBUTES ALIGN : 64 :: x4_loc
real(sp), allocatable :: x4_loc(:)   ! k4x coordinates around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: y4_loc
real(sp), allocatable :: y4_loc(:)   ! k4y coordinates around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: ds_loc
real(sp), allocatable :: ds_loc(:)   ! step size around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: jac_loc
real(sp), allocatable :: jac_loc(:)  ! jacobian term around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: cple_loc
real(sp), allocatable :: cple_loc(:) ! coupling coefficient around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: sym_loc
real(sp), allocatable :: sym_loc(:)  ! factor for symmetry between k3 and k4
!
!DIR$ ATTRIBUTES ALIGN : 64 :: k_pol
real(sp), allocatable :: k_pol(:)  ! wave numbers during polar generation of locus
!DIR$ ATTRIBUTES ALIGN : 64 :: c_pol
real(sp), allocatable :: c_pol(:)  ! cosines during polar generation of locus
!DIR$ ATTRIBUTES ALIGN : 64 :: a_pol
real(sp), allocatable :: a_pol(:)  ! angles of polar locus
!
!  characteristics of modified locus, result
!
!DIR$ ATTRIBUTES ALIGN : 64 :: x2_mod
real(sp), allocatable :: x2_mod(:)   ! k2x coordinates along locus
!DIR$ ATTRIBUTES ALIGN : 64 :: y2_mod
real(sp), allocatable :: y2_mod(:)   ! k2y coordinates along locus
!DIR$ ATTRIBUTES ALIGN : 64 :: x4_mod
real(sp), allocatable :: x4_mod(:)   ! k4x coordinates along locus
!DIR$ ATTRIBUTES ALIGN : 64 :: y4_mod
real(sp), allocatable :: y4_mod(:)   ! k4y coordinates along locus
!DIR$ ATTRIBUTES ALIGN : 64 :: z_mod
real(sp), allocatable :: z_mod(:)    ! data value around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: s_mod
real(sp), allocatable :: s_mod(:)    ! coordinate along locus
!DIR$ ATTRIBUTES ALIGN : 64 :: ds_mod
real(sp), allocatable :: ds_mod(:)   ! step size around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: jac_mod
real(sp), allocatable :: jac_mod(:)  ! jacobian term around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: cple_mod
real(sp), allocatable :: cple_mod(:) ! coupling coefficient around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: sym_mod
real(sp), allocatable :: sym_mod(:)  ! factor for symmetry between k3 and k4
!
!DIR$ ATTRIBUTES ALIGN : 64 :: k2m_mod
real(sp), allocatable :: k2m_mod(:) ! k2 magnitude around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: k2a_mod
real(sp), allocatable :: k2a_mod(:) ! k2 angle around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: k4m_mod
real(sp), allocatable :: k4m_mod(:) ! k4 magnitude around locus
!DIR$ ATTRIBUTES ALIGN : 64 :: k4a_mod
real(sp), allocatable :: k4a_mod(:) ! k4 angle around locus
!
!   result of subroutine Q_weight
!

!DIR$ ATTRIBUTES ALIGN : 64 :: wk_k2
real(sp), allocatable :: wk_k2(:)   ! position of k2 and k4 wave number
!DIR$ ATTRIBUTES ALIGN : 64 :: wk_k4
real(sp), allocatable :: wk_k4(:)   ! w.r.t. discrete k-grid
!DIR$ ATTRIBUTES ALIGN : 64 :: wa_k2
real(sp), allocatable :: wa_k2(:)   ! position of k2 and k4 wave number
!DIR$ ATTRIBUTES ALIGN : 64 :: wa_k4
real(sp), allocatable :: wa_k4(:)   ! w.r.t. discrete a-grid
!DIR$ ATTRIBUTES ALIGN : 64 :: wt_k2
real(sp), allocatable :: wt_k2(:)   ! weight factor in tail,
!DIR$ ATTRIBUTES ALIGN : 64 :: wt_k4
real(sp), allocatable :: wt_k4(:)   ! wt==1 for wave numbers inside k-grid
!
!DIR$ ATTRIBUTES ALIGN : 64 :: t_ik2
integer(int4), allocatable :: t_ik2(:)   ! transformed weight for k2-magnitude
!DIR$ ATTRIBUTES ALIGN : 64 :: t_ia2
integer(int4), allocatable :: t_ia2(:)   ! transformed direction for k2
!DIR$ ATTRIBUTES ALIGN : 64 :: t_ik4
integer(int4), allocatable :: t_ik4(:)   ! transformed tail factor for k2
!DIR$ ATTRIBUTES ALIGN : 64 :: t_ia4
integer(int4), allocatable :: t_ia4(:)   ! transformed weight for k4
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w1k2
real(sp), allocatable :: t_w1k2(:)  ! transformed weight 1 for k2
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w2k2
real(sp), allocatable :: t_w2k2(:)  ! transformed weight 2 for k2
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w3k2
real(sp), allocatable :: t_w3k2(:)  ! transformed weight 3 for k2
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w4k2
real(sp), allocatable :: t_w4k2(:)  ! transformed weight 4 for k2
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w1k4
real(sp), allocatable :: t_w1k4(:)  ! transformed weight 1 for k4
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w2k4
real(sp), allocatable :: t_w2k4(:)  ! transformed weight 2 for k4
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w3k4
real(sp), allocatable :: t_w3k4(:)  ! transformed weight 3 for k4
!DIR$ ATTRIBUTES ALIGN : 64 :: t_w4k4
real(sp), allocatable :: t_w4k4(:)  ! transformed weight 4 for k4
!DIR$ ATTRIBUTES ALIGN : 64 :: t_zz
real(sp), allocatable :: t_zz(:)    ! product term
!DIR$ ATTRIBUTES ALIGN : 64 :: t_tail2
real(sp), allocatable :: t_tail2(:) ! tail factor for k2
!DIR$ ATTRIBUTES ALIGN : 64 :: t_tail4
real(sp), allocatable :: t_tail4(:) ! tail factor for k4
!DIR$ ATTRIBUTES ALIGN : 64 :: t_tail4
real(sp), allocatable :: t_sym(:)   ! transformed symetry factor
!DIR$ ATTRIBUTES ALIGN : 64 :: t_cple
real(sp), allocatable :: t_cple(:)  ! transformed coupling coefficient
!DIR$ ATTRIBUTES ALIGN : 64 :: t_jac
real(sp), allocatable :: t_jac(:)   ! tranformed jacobian term
!DIR$ ATTRIBUTES ALIGN : 64 :: t_ws
real(sp), allocatable :: t_ws(:)    ! transformed weighted/step size
!
!  corresponding declarations
!
!DIR$ ATTRIBUTES ALIGN : 64 :: r_ik2
integer(int4), allocatable :: r_ik2(:)
!DIR$ ATTRIBUTES ALIGN : 64 :: r_ia2
integer(int4), allocatable :: r_ia2(:)
!DIR$ ATTRIBUTES ALIGN : 64 :: r_ik4
integer(int4), allocatable :: r_ik4(:)
!DIR$ ATTRIBUTES ALIGN : 64 :: r_ia4
integer(int4), allocatable :: r_ia4(:)
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w1k2
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w2k2
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w3k2
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w4k2
real(sp), allocatable :: r_w1k2(:),r_w2k2(:),r_w3k2(:),r_w4k2(:)
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w1k4
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w2k4
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w3k4
!DIR$ ATTRIBUTES ALIGN : 64 :: r_w4k4
real(sp), allocatable :: r_w1k4(:),r_w2k4(:),r_w3k4(:),r_w4k4(:)
!DIR$ ATTRIBUTES ALIGN : 64 :: r_zz, r_jac, r_cple, r_sym, r_ws
real(sp), allocatable :: r_zz(:),r_jac(:),r_cple(:),r_sym(:),r_ws(:)
!DIR$ ATTRIBUTES ALIGN : 64 :: r_tail2, r_tail4
real(sp), allocatable :: r_tail2(:),r_tail4(:)
!
!DIR$ ATTRIBUTES ALIGN : 64 :: dt13
real(sp), allocatable :: dt13(:)    ! increment along locus
!
   !DIR$ ATTRIBUTES ALIGN : 64 :: q_xk
real(sp), allocatable :: q_xk(:)    ! extended wave number array starting at index 0
!DIR$ ATTRIBUTES ALIGN : 64 :: q_sk
real(sp), allocatable :: q_sk(:)    ! step size of extended wave number array
real(sp) :: sk_max                     ! maximum wave number in extended array
!
!DIR$ ATTRIBUTES ALIGN : 64 :: q_k
real(sp), allocatable :: q_k(:)     !  wave number grid [1/m]
!DIR$ ATTRIBUTES ALIGN : 64 :: q_dk
real(sp), allocatable :: q_dk(:)    !  width of wave number bins [1/m]
!DIR$ ATTRIBUTES ALIGN : 64 :: q_kpow
real(sp), allocatable :: q_kpow(:)  !  wave number to a certain power, used in filtering
!DIR$ ATTRIBUTES ALIGN : 64 :: q_f
real(sp), allocatable :: q_f(:)     !  frequencies accociated to wave number/depth
!DIR$ ATTRIBUTES ALIGN : 64 :: q_df
real(sp), allocatable :: q_df(:)    !  step size of frequency grid
!DIR$ ATTRIBUTES ALIGN : 64 :: q_sig
real(sp), allocatable :: q_sig(:)   !  radian frequencies associated to wave number/depth
!DIR$ ATTRIBUTES ALIGN : 64 :: q_dsig
real(sp), allocatable :: q_dsig(:)  !  step size of radian frequency grid
!DIR$ ATTRIBUTES ALIGN : 64 :: q_cg
real(sp), allocatable :: q_cg(:)    !  group velocity (m/s)
!DIR$ ATTRIBUTES ALIGN : 64 :: q_a
real(sp), allocatable :: q_a(:)     !  directions of quadruplet grid in radians
!DIR$ ATTRIBUTES ALIGN : 64 :: q_ad
real(sp), allocatable :: q_ad(:)    !  directions of quadruplet grid in degrees
!DIR$ ATTRIBUTES ALIGN : 64 ::   a
real(sp), allocatable :: a(:,:)     !  Action density on wave number grid A(sigma,theta)
!DIR$ ATTRIBUTES ALIGN : 64 :: nspec
real(sp), allocatable :: nspec(:,:) !  Action density on wave number grid N(kx,ky)
!DIR$ ATTRIBUTES ALIGN : 64 :: nk1d
real(sp), allocatable :: nk1d(:)    !  Internal 1d action density spectrum N(k)
!DIR$ ATTRIBUTES ALIGN : 64 :: qnl
real(sp), allocatable :: qnl(:,:)   !  Nonlinear energy transfer Snl(k,theta)
!
integer(int4) :: id_facmax                   ! Factor for determining range of depth search (Q_SEARCHGRID)
real(sp) :: q_dird1,q_dird2                ! first and last direction of host model (via XNL_INIT) degrees
real(sp) :: q_depth                        ! local water depth in m
real(sp) :: q_maxdepth                     ! maximum water depth, set in XNL_INIT, used in Q_CTRGRID
real(sp) ::  q_mindepth                     ! minimum water depth, set in XNL_INIT, used in Q_CTRGRID
real(sp) :: q_lambda                       ! geometric scaling factor for 'deep' water loci
real(sp) :: q_scale                        ! additional scale factor resulting from SEARCH for neasrest grid
!
real(sp) :: eps_q                          ! absolute accuracy for check of Q
real(sp) :: eps_k                          ! absolute accuracy for equality check of k
real(sp) :: rel_k                          ! relative accuracy for equality check of k
!
integer(int4) :: iq_stack                    ! Sequence number of stack with subroutine calls
character(len=21) cstack(mq_stack)  ! Stack with module names
!
!  characteristics of locus
!
real(sp) :: crf1     ! estimated circumference of locus
!---------------------------------------------------------------------------------
!
!  information about type of grid
!
integer(int4) :: iaref       !  index of first angle of reference wave numbers
integer(int4) :: iamax       !  maximum difference in indices for sector grids
integer(int4) :: iaq1,iaq2   !  indices of do-loop for directions
integer(int4) :: iag1,iag2   !  range of directions for precomputed interaction grid
real(sp) :: q_ang1,q_ang2  !  lower and upper angle of grid in degrees
real(sp) :: q_delta        !  directional spacing of angular grid in radians
real(sp) :: q_deltad       !  directional spacing of angular grid in degrees
!
real(sp) :: q_ffac         !  geometric factor between subsequent frequencies
real(sp) :: q_kfac         !  geometric factor between subsequent wave numbers
                    !  (only valid for IQ_IDISP==1)
real(sp) :: qk_tail        ! power of spectral tail of N(k), computed from qf_tail
real(sp) :: ff_tail        ! fraction of maximum frequency where parametric tail starts
!
!-----------------------------------------------------------------------------
!
...
!============== General settings =================
!
integer(int4) :: iq_type   !  method for computing the nonlinear interactions
...
!
integer(int4) :: iq_err    !  counts the number of errors
...
!
integer(int4) :: iq_warn   !  counts the number of warnings
!
!  indices for test output of actual integration
!  these values are set and optionally modified in Q_SETCONFIG
!
integer(int4) :: mk_a,mk_b  ! indices of k1&k3     when test output is needed
integer(int4) :: ma_a,ma_b  ! indices of dir1&dir3 when test output is needed
contains
!----------------------------------------------------------------------------------
!------------------------------------------------------------------------------
...
! Local parameters
!
integer(int4) :: iuerr     ! error indicator
integer(int4) :: idepth    ! index over water depths
integer(int4) :: igrid     ! status of quadruplet grid file
integer(int4) :: ia,ik     ! counters
!
real(sp) :: depmin       ! minimum water depth
real(sp) :: dstep        ! directional step
real(sp) :: dgap         ! directional gap between first and last direction
!
...
q_dstep    = 0.1_sp               ! Set minimum step for coding depth files
...
!
fqmin = sigma(1)/(4._sp*pih)      ! minimum frequency in Hz
fqmax = sigma(nsigma)/(4._sp*pih) ! maximum frequency in Hz
...
dgap = 180._sp- abs(180._sp- abs(dird(1)-dird(ndir)))  ! directional gap
!
!----------------------------------------------------------------------
...
!     local variables
!
integer(int4), save :: i_qmain    ! counter number of calls of XNL_MAIN
integer(int4) i_qlast             ! value of iquad in last call
!
integer(int4) :: isig         ! counter for sigma values
integer(int4) :: idir         ! counter of directions
real(sp) :: q_dfac          ! depth scale factor for nonlinear transfer
!
real(sp) :: sum_e           ! sum of energy
real(sp) :: sum_a           ! sum of action
real(sp) :: sum_mx          ! sum of momentum in x-direction
real(sp) :: sum_my          ! sum of momentum in y-direction
!
...
!     Local variables
!
integer(int4) :: ia,isig      ! counters for directions and wave numbers
integer(int4) :: isig_tail    ! index of bin where tail starts
!
!DIR$ ATTRIBUTES ALIGN : 64 :: xnl_tail
real(sp), dimension(nsig) :: xnl_tail  ! tail factors for transfer rate
!DIR$ ATTRIBUTES ALIGN : 64 ::  diag_tail
real(sp), dimension(nsig) :: diag_tail ! tail factors for diagonal term
real(sp) :: x_tail          ! power in tail of nonlinear transfer rate
real(sp) :: d_tail          ! power in tail of diagonal term
real(sp) :: sig_tail        ! threshold sigma for start of parametric tail
!-----------------------------------------------------------------------------
!
...
!  Local variables
!-------------------------------------------------------------------------------
integer(int4) :: maq   ! number of theta elements in grid matrix
integer(int4) :: mkq   ! number of k-elements in grid matrix
!-------------------------------------------------------------------------------
...
!Type   I/O          Name     Description
!
integer(int4), intent(in) :: nk         ! number of wave numbers
integer(int4), intent(in) :: ndir       ! number of directions
real(sp), intent(in)  :: xnl(nk,ndir) ! transfer rate
real(sp), intent(out) :: sum_e        ! sum of wave energy
real(sp), intent(out) :: sum_a        ! sum of wave action
real(sp), intent(out) :: sum_mx       ! sum of momentum in x-direction
real(sp), intent(out) :: sum_my       ! sum of momentum in y-direction
!
!  4. Error messages
!
!  5. Called by:
...
!     Local variables
!
real(sp) :: aa    ! action density
real(sp) :: ee    ! energy density
real(sp) :: kk    ! wave number
real(sp) :: momx  ! momentum in x-direction
real(sp) :: momy  ! momentum in y-direction
real(sp) :: qq    ! bin size
!
integer(int4) :: ia ! counter over directions
integer(int4) :: ik ! counter over wave numbers
!
!------------------------------------------------------------------------------
!
...
! Type    I/O          Name       Description
!----------------------------------------------------------------------
real(sp), intent(in)  ::   k1x     !  x-component of wave number vector k1
real(sp), intent(in)  ::   k1y     !  y-component of wave number vector k1
real(sp), intent(in)  ::   k2x     !  x-component of wave number vector k2
real(sp), intent(in)  ::   k2y     !  y-component of wave number vector k2
real(sp), intent(in)  ::   k3x     !  x-component of wave number vector k3
real(sp), intent(in)  ::   k3y     !  y-component of wave number vector k3
real(sp), intent(in)  ::   k4x     !  x-component of wave number vector k4
real(sp), intent(in)  ::   k4y     !  y-component of wave number vector k4
real(sp), intent(in)  ::   dep     !  depth in m
real(sp), intent(out) ::   sum_kx  !  sum of x-components of quadruplet
real(sp), intent(out) ::   sum_ky  !  sum of y-components of quadruplet
real(sp), intent(out) ::   sum_w   !  sum of radian frequencies
!
!  4. Error messages
!
!  5. Subroutines used
...
!     Local variables
!
real(sp) :: w1,w2,w3,w4            ! radian frequecies of wave numbers
...
!Type   I/O          Name          Description
!----------!----------------------------------------------------------------------------
real(sp), intent(out) :: ka,kb       ! lowest and highest wave number magnitude of k2-locus
real(sp), intent(out) :: km          ! wave number magnitude at mid point
real(sp), intent(out) :: kw          ! half width of locus
real(sp), intent(out) :: loclen      ! estimated length of locus
!
!  4. Error messages
!
!  5. Called by:
...
!     Local variables
!-------------------------------------------------------------------------------
real(sp) :: k1m          ! magnitude of wave number k1
real(sp) :: k3m          ! magnitude of wave number k3
real(sp) :: pcos,psin    ! cosine and sine of normalize angle of P
real(sp) :: klen         ! total length of line locus for case w1=w3
!
real(sp) :: kx_beg       ! x-component at start point
real(sp) :: ky_beg       ! y-component at start point
real(sp) :: kx_end       ! x-component at end point
real(sp) :: ky_end       ! y-component at end point
!
real(sp) :: dsp,dsm      ! distances in plus and minus direction
real(sp) :: sum          ! total length of locus
!
real(sp) :: w1,w3        ! radian frequencies of wave numbers k1 and k3
real(sp) :: eps          ! local accuracy for determination of roots
real(sp) :: area1        ! area of locus as computed
real(sp) :: area2        ! area of locus as based on LOCPOS and ellipse
real(sp) :: ratio        ! maximum ratio between area1 and area2
!
integer(int4) :: ierr      ! local error level
integer(int4) :: iloc,jloc ! counters along locus
integer(int4) :: itest     ! local test level for test output to unit luqtst
integer(int4) :: ip1       ! index +1
integer(int4) :: im1       ! index -1
integer(int4) :: jj        ! counter
!------------------------------------------------------------------------------
...
!  set initial values
!
eps     = 10._sp*epsilon(1.)     ! set accuracy 10 times machine accuracy
...
!  3. Parameters used
!
integer(int4), intent(in)  :: itask  !  task to perform by Q_CTRGRID
!                                 ==1: read and check header block
!                                 ==2: read and write grid file, according to
!                                      setting of IQ_MAKE
integer(int4), intent(out) :: igrid  !  status of grid checking
...
!     Local variables
!
integer(int4) :: iaz,ikz,jaz,jkz                ! counters for checking header of BQF file
integer(int4) :: iz_geom,iz_disp,iz_cple        ! values as read in BQF file
integer(int4) :: naz                            ! number of directions in BQF file
integer(int4) :: nkz                            ! number of wave numbers in BQF file
integer(int4) :: idep,jdep                      ! coding for depth in BQF file
!
logical(int4) :: lbqf                           ! flag for existence of BQF file
real(sp) :: s_depth                           ! (stepped) depth
real(sp) :: q_depth_saved                     ! save input water depth, needed after call to Q_MAKEGRID
real(sp) :: z_depth                           ! water depth in BQF file
!DIR$ ATTRIBUTES ALIGN : 64 :: z_ad,z_sig
real(sp) , allocatable :: z_ad(:),z_sig(:)  ! directions and radian frequencies of grid in BQF file
integer(int4) :: ierr,iuerr                     ! error variables
!------------------------------------------------------------------------------
!
...
!---------------------------------------------------------------------------------------
  if(.not. lq_grid) then
az: do iaz = 1,naz
      if(abs(q_ad(iaz)-z_ad(iaz)) > 0.01_sp) then
...
!
  if(.not. lq_grid) then
ak: do ikz = 1,nkz
      if(abs(q_sig(ikz)-z_sig(ikz)) > 0.01_sp) then
...
!
  if(abs(z_depth-s_depth) > 0.09_sp .and. iq_disp > 1 .and. .not. lq_grid) then
...
! Type          I/O     Name           Description
!-------------------------------------------------------------------------
integer(int4), intent (in) :: nsig          ! Number of sigma-values
integer(int4), intent (in) :: nang          ! Number of directions
real(sp), intent(in)     :: n(nsig,nang)  ! N(nsig,nang) Action density
real(sp), intent(in)     :: sigma(nsig)   ! sigma values
real(sp), intent(in)     :: angle(nang)   ! directions in (radians)
real(sp), intent(in)     :: depth         ! Depth (m)
real(sp), intent(in)     :: grav          ! Gravitational acceleration
real(sp), intent(out)    :: q_dfac        ! scale factor
!
!  4. Error messages
!
!  5. Called by:
...
!     local variables
!
real(sp) :: w          ! radian frequency
real(sp) :: kk         ! local wave number
real(sp) :: sqkk       ! square root of local wave number
real(sp) :: dnn        ! summation quantity
real(sp) :: kms        ! mean wave number
real(sp) :: kd         ! depth*mean wave number product
real(sp) :: sum0       ! summation variable for total energy
real(sp) :: sumk       ! summation variable for wave number
real(sp) :: delta      ! directional step, in radians
!
integer(int4) :: isig    ! counter over sigma loop
integer(int4) :: iang    ! counter over direction loop
!
! functions
...
! temporary data
!
!DIR$ ATTRIBUTES ALIGN : 64 :: dsigma
real(sp), dimension(nsig) :: dsigma ! step size of sigma array, used for integration
!------------------------------------------------------------------------------
!
...
if(sum0 > 0) then
  kms = (sum0/sumk)**2
  kd = max(0.5_sp,0.75_sp*kms*depth)
  q_dfac = 1+5.5_sp/kd*(1._sp-5._sp/6._sp*kd)*exp(-5._sp/4._sp*kd)
...
integer(int4) :: ntext           ! number of text line
integer(int4) :: iend            ! indicator for end of line
integer(int4) :: iutxt           ! unit number for text file
integer(int4) :: iuerr           ! indicator for error
integer(int4) :: j_stack         ! counter in printing stack
integer(int4) :: ispace          ! indicates that first character of line is space
!
tempfile=trim(qbase)//'.err'
...
!Type      I/O           name       Description
!------------------------------------------------------------------------------
integer(int4), intent(in)  ::  ik1    !  k-index of wave number k1
integer(int4), intent(in)  ::  ia1    !  theta-index of wave number k1
integer(int4), intent(in)  ::  ik3    !  k-index of wave number k3
integer(int4), intent(in)  ::  ia3    !  theta-index of wave number k3
integer(int4), intent(out) :: ifnd    !  indicator if reference locus exists in database
!
!  4. Error messages
!
!  5. Called by
...
!     Local variables
!
integer(int4) :: it1,it3           ! work indices for directions, copy of ia1 and ia3
integer(int4) :: idir              ! switch to indicate if locus should be inverted
integer(int4) :: itrans            ! type of transformation
integer(int4) :: iloc,jloc         ! counters along locus
integer(int4) :: iadif,ikdif       ! difference in angular and k-index
integer(int4) :: ja1r,ja3r         ! indices for reference locus
integer(int4) :: ja2r,ja4r         ! directional indices for reference locus
integer(int4) :: imirror           ! extra step when locus is mirrorred
!
integer(int4) :: ibeta,kdif
integer(int4) :: nloc             ! number of points on locus
!
integer(int4) :: ierr
integer(int4) :: kmem                 ! index for storing 2-d matrix in 1-d array
integer(int4) :: amem                 ! index for storing direction of reference wave number k3
!
real(sp) :: lambda                  ! geometric scale factor
real(sp) :: j_lambda                ! scale factor for Jacobian term
real(sp) :: c_lambda                ! scale factor for coupling coefficient
real(sp) :: zz_lambda               ! combined scale factor
!
!DIR$ ATTRIBUTES ALIGN : 64 :: xr2,yr2
real(sp), dimension(nlocus) :: xr2,yr2 ! xy-components of reference k2-locus
!DIR$ ATTRIBUTES ALIGN : 64 :: xt2,yt2
real(sp), dimension(nlocus) :: xt2,yt2 ! xy-components of test k2-locus
!DIR$ ATTRIBUTES ALIGN : 64 :: xt4,yt4
real(sp), dimension(nlocus) :: xt4,yt4 ! xy-components of test k4-locus
real(sp) :: wk,wa,vk,va
!! \A
!! real x_kfunc                 ! real function to compute wave number
!! \Z
!
integer(int4) :: ikmin,ja1,ja3,jk1,jk3,itmin
integer(int4) :: ibdif,nhalf
integer(int4) :: iaq,ikq              ! counters for loop over direction and wave numbers
!------------------------------------------------------------------------------
!
...
!     Local variables
!
integer(int4) :: iaq,ikq    ! counters for loops over directions and wave numbers
real(sp) :: ff            ! frequency
!!!real z_wnumb       ! service function to compute wave number
!
integer(int4) :: iuerr      ! error indicator for i/o
!------------------------------------------------------------------------------
!
call q_stack('+q_init')
!
...
!
eps_q   = 0.001_sp           ! absolute accuracy for check of q==0
eps_k   = 1.e-5_sp           ! absolute accuracy for equality check of k
rel_k   = 0.001_sp           ! relative accuracy for equality check of k
!
sk_max = 500._sp             ! set maximum waver number
wk_max = real(nkq+0.9999_sp) ! set maximum wave number index
!
...
!
q_ffac = (fqmax/fqmin)**real(1._sp/(nkq-1._sp))         ! geometric spacing factor of frequencies
!
...
if(iq_grid==1 .or. iq_grid==2) then    ! define symmetric sector
  q_deltad = 2._sp*q_sector/real(naq-1._sp)  ! delta in degrees
...
elseif(iq_grid==3) then                ! full sector
  q_deltad = 360._sp/real(naq)            ! degrees
  q_ang1 = 0                           ! degrees
  q_ang2 = 360._sp-q_delta                ! degrees
end if
!
q_delta = q_deltad*dera                ! directional step in radians
ncirc   = 2.00001_sp*2._sp*pih/q_delta       ! number of directions on circle
!
if(iq_prt >= 2) then
...
!Type    I/O          Name     Description
!-----------------------------------------------------------------
real(sp), intent (out) :: ka     ! minimum k along symmetry axis
real(sp), intent (out) :: kb     ! maximum k along symmetry axis
real(sp), intent (out) :: km     ! wave number at midpoint
real(sp), intent (out) :: kw     ! half width of locus at midpoint
real(sp), intent (out) :: loclen ! estimated length of locus
!
!  4. Error messages
!
...
!     Local variables
!
real(sp) :: kp           ! wave number at peak
real(sp) :: kpx,kpy      ! wave number at peak maximum
real(sp) :: zp           ! value of locus function at maximum
real(sp) :: za,zb        ! (test) value of locus function at kmin & kmax
real(sp) :: zz1,zz2      ! intermediate function values in interation process
real(sp) :: kk1,kk2      ! start values for finding root of locus equation
real(sp) :: kk1x,kk1y    ! wave number components at one side of root
real(sp) :: kk2x,kk2y    ! wave number components at other side of root
real(sp) :: beta1,beta2  ! parameters specifying cross component
real(sp) :: betaw        ! parameter specifying iterated cross component
real(sp) :: kwx,kwy      ! wave number at side of locus
real(sp) :: zw           ! function value at (kwx,kwy)
real(sp) :: a1,a2,b1,b2  ! constants in polynomial approximation of elliptic function
real(sp) :: aa,bb,mm,mm1 ! semi-major exis of ellips and derived parameters
!
real(sp) :: eps         ! local machine accuracy for determination of roots
real(sp) :: bacc        ! accuracy for determination of beta
real(sp) :: kacc        ! accuracy for determination of wave number roots
real(sp) :: qs          ! (w1-w3)/sqrt(g)
real(sp) :: qsq         ! gs^2
!
...
!
integer(int4) :: ierr     ! local error indicator, used in function Z-ZERO1
integer(int4) :: itest    ! local test level for test output
integer(int4) :: lutest   ! unit for test output in service routines
integer(int4) :: iter     ! local iteration number
integer(int4) :: maxiter  ! maximum number of iteration for determining starting points
!
!  function declarations
...
!  set initial values
!
eps     = epsilon(1._sp)         ! determine machine accurcy
...
if(iq_disp==1) then
  ...
  qsq  = qs*qs
  if(qs < 0) then
    ka = 0.5_sp*(-qs+sqrt(2.0_sp*pmag-qsq))
    ka = ka**2
    kb = (pmag+qsq)/(2._sp*qs)
    kb = kb**2
    za = x_locus1(ka)
    zb = x_locus1(kb)
  else
...
!   search root by Ridder's method
!
    kacc = 10._sp*max(abs(kk1),abs(kk2))*eps
    ka = z_root2(x_locus1,kk1,kk2,kacc,lutest,ierr)
...
!
if(q < 0) then
  kmidx = kmid*cos(pang+2._sp*pih)
  kmidy = kmid*sin(pang+2._sp*pih)
else
  kmidx = kmid*cos(pang)
  kmidy = kmid*sin(pang)
end if
...
!
bacc = 10._sp*max(beta1,beta2)*eps
...
!
mm1 = 1._sp-mm
a1 = 0.4630151_sp;  a2 = 0.1077812_sp;
b1 = 0.2452727_sp;  b2 = 0.0412496_sp;
!
if (mm1==0) then
  loclen = 4._sp*max(aa,bb)
else
  loclen = 4._sp*max(aa,bb)*((1._sp + a1*mm1 + a2*mm1**2) + (b1*mm1 + b2*mm1**2)*log(1/mm1))
end if
!
if(itest >= 1) then
...
!     Local variables
!
integer(int4) :: iloc,jloc            ! counters
integer(int4) :: iaq,ikq              ! counters
integer(int4) :: iaq3,ikq1,ikq3,nkq1  ! counters
integer(int4) :: jaq1,jaq3            ! counters
integer(int4) :: amem,kmem            ! index of angle and wave number in grid
real(sp) :: aa1,aa3,kk1,kk3         ! temporary wave number variables
!
integer(int4) :: nzloc                ! counter for non-zero contributions along locus
integer(int4) :: nztot1,nztot2        ! total number of zero and non-zero points on locus
integer(int4) :: ik2,ia2              ! index of wave number k2
integer(int4) :: ik4,ia4              ! index of wave number k4
!
real(sp) :: wk,wa                   ! weights
real(sp) :: w1k2,w2k2,w3k2,w4k2     ! interpolation weights
real(sp) :: w1k4,w2k4,w3k4,w4k4     ! interpolation weights
!
real(sp) :: ka,kb       ! lower and higher wave number magnitude
real(sp) :: km          ! wave number at mid point
real(sp) :: kw          ! half width of locus
!
real(sp) :: tfac        ! combined tail factor
!
logical lwrite   ! indicator if binary interaction grid has been written successfully
real(sp) :: smax        ! maximum s-value
!
!DIR$ ATTRIBUTES ALIGN : 64 :: xloc,yloc
real(sp), allocatable, dimension(:) :: xloc,yloc
real(sp) :: qq
!-------------------------------------------------------------------------------
...
!     Local parameters
!
integer(int4) :: ierr,jerr    ! error indicators
integer(int4) :: nold,nnew    ! old and new number of points on locus
integer(int4) :: iold,inew    ! counter for loop along points
integer(int4) :: iloc         ! counter for loop along locus
integer(int4) :: jloc         ! counter for loop over lumped locus
integer(int4) :: itest        ! local test level, by default equal to IQ_TEST
!
real(sp) :: k2a,k2m         ! angle (deg) and wave number magnitude of wave number k2
real(sp) :: k4a,k4m         ! angle (deg) and wave number magnitude of wave number k4
real(sp) :: w2,w4           ! radian frequencies of wave numbers
!
!
real(sp) :: dk13,dk14       ! difference wave number
real(sp) :: dsnew,slen      ! new step size and length of locus
real(sp) :: zero            ! 0
real(sp) :: q_eps           ! accuracy to distinguish special case, with q=0
real(sp) :: diold           ! 'real' old number of indices between succeeding lumped bins
real(sp) :: dinew           ! 'real' new number of indices between succeeding lumped bins
!
!!real x_disper        ! evaluate dispersion relation
!DIR$ ATTRIBUTES ALIGN : 64 :: sold
real, allocatable, dimension(:) :: sold     ! old coordinate along locus
!DIR$ ATTRIBUTES ALIGN : 64 :: snew
real, allocatable, dimension(:) :: snew    ! new coordinate along locus
!--------------------------------------------------------------------------
...
!
      do iloc=1,nold-1
        jloc = floor((iloc-1._sp)*diold/dinew + 1.49999_sp)
...
!Type           I/O          name          description
!-------------------------------------------------------
integer(int4), intent(inout)  ::   ik       !  Index of wave number
integer(int4), intent(inout)  ::   ia       !  index of angle
real(sp), intent(inout)     ::   w1       !  weight of first corner point
real(sp), intent(inout)     ::   w2       !  weight of second corner point
real(sp), intent(inout)     ::   w3       !  weight of third corner point
real(sp), intent(inout)     ::   w4       !  weight of fourth corner point
!
!  4. Error messages
!
!  5. Called by
...
!     Local parameters
!
integer(int4) :: ik_max   ! k-index with maximum weight
integer(int4) :: ia_max   ! theta index with maximum weight
integer(int4) :: iw_max   ! index with highest weight
real(int4) :: w_max       ! maximum weight
!------------------------------------------------------------------------------
...
!Type    I/O        Name             Description
!------------------------------------------------------------------------------
real(sp), intent(in) :: kmin           ! minimum wave number on locus
real(sp), intent(in) :: kmax           ! maximum wave number on locus
real(sp), intent(in) :: kx_beg         ! x-coordinate of begin point
real(sp), intent(in) :: ky_beg         ! y-coordinate of begin point
real(sp), intent(in) :: kx_end         ! x-coordinate of end point
real(sp), intent(in) :: ky_end         ! y-coordinate of end point
real(sp), intent(in) :: loclen         ! estimated length of locus
integer(int4), intent (out)  :: ierr     ! error condition
!
...
!     Local variabels
!
integer(int4) :: ipol      ! counter
integer(int4) :: jpol      ! counter
integer(int4) :: iend      ! indicates end of locus computation
integer(int4) :: ipass     ! counter for passes
integer(int4) :: npol      ! number of points on locus
integer(int4) :: npass     ! number of passes in iteration process
integer(int4) :: mpol      ! maximum number of points on locus, related to MLOCUS
!
real(sp) :: kold         ! temporary wave number
real(sp) :: knew         ! temporary wave number
real(sp) :: cosold       ! 'old' cosine of angle
real(sp) :: cosnew       ! 'new' cosine of angle
real(sp) :: dkpol        ! step in wave number
real(sp) :: dkold        ! 'old' step in wave number
real(sp) :: ang1         ! 'old' angle
real(sp) :: ang2         ! 'new' angle
real(sp) :: kk1          ! 'old' wave number
real(sp) :: kk2          ! 'new' wave number
real(sp) :: kratio       ! ratio between succesive k-values when IQ_LOCUS=3
real(sp) :: arg          ! argument
real(sp) :: dk           ! step in wave number
real(sp) :: dke          ! estimate of new dk
real(sp) :: dsnew        ! new step size along locus
real(sp) :: dsz          ! estimated step size along locus
!
integer(int4) :: itest    ! local test level
integer(int4) :: lutest   ! unit number for test output in service routine
!
!  function declarations
...
!----------------------------------------------------------------------------------------------
    ...
    kold        = knew
    cosold      = cosnew
    if (abs(dkold) < 0.0005_sp*(kmax-kmin)) iend=1
  end do
!
...
!Type, I/O               Name      Description
!--------------------------------------------------------------------------
integer(int4), intent(in) ::   iquad   ! Indicator for a specific choice of
...
! Local variables
!
integer(int4) :: iend               ! indicator for end of file
integer(int4) :: iuerr              ! error status of file io
character(len=10) :: cpar     ! character parameter
real(sp) :: rpar                  ! real parameter
!--------------------------------------------------------------------------------
!
...
!-------------------------------------------------------------------------------
qf_krat = 2.5_sp             ! maximum ratio between wave numbers k1 and k3
qf_dmax = 75.0_sp            ! difference in degrees between k1 and k3
qf_frac = 0.1_sp             ! fraction of maximum energy density
!
q_sector = 120._sp           ! set size of half-plane direction sector (120)
!-------------------------------------------------------------------------------
! setting for parametric tail
!--------------------------------------------------------------------------------
ff_tail  = 0.75_sp           ! parametric tail starts at 0.75 of maximum frequency
!------------------------------------------------------------------------------
!
...
!  3. Parameters used
!
real(sp), intent(in)     :: depth  !  depth for which grid file must be found
integer(int4), intent(out) :: igrid  !  status of grid checking
!                                 ==0: a proper grid exists
!                                 ==1: grid file does not exist
!                                 ==2: grid file exists, but it is incorrect
...
!     Local variables
!
integer(int4) :: id        ! counter
integer(int4) :: idepth    ! integer depth
integer(int4) :: id_upper  ! upper limit of search
integer(int4) :: id_lower  ! lower limit of depth search
!
real(sp) :: d_lower      ! lower valid depth
real(sp) :: d_upper      ! upper valid depth
real(sp) :: r_lower      ! ratio with lower valid depth
real(sp) :: r_upper      ! ratio with upper valid depth
real(sp) :: s_depth      ! target depth in m, saved in this variable
real(sp) :: dfac1,dfac2  ! depth scale factors
real(sp) :: eps          ! accuracy
!------------------------------------------------------------------------------
!
...
integer(int4) :: mod_len                 ! length of mod_name
!
!!\A
if(iq_trace > 0) then
...
! Type   i/o             Name           Description
!----------------------------------------------------------------------------------
integer(int4), intent(in)   :: nloc         ! number of points in array with wave number
real(sp), intent(in)      :: k1x          ! x-component  of wave number k1
real(sp), intent(in)      :: k1y          ! y-component  of wave number k1
real(sp), intent(in)      :: k3x          ! x-component  of wave number k3
real(sp), intent(in)      :: k3y          ! y-component  of wave number k3
real(sp), intent(in)      :: k4x(nloc)    ! x-components of wave number k4
real(sp), intent(in)      :: k4y(nloc)    ! y-components of wave number k4
real(sp), intent(out)     :: symfac(nloc) ! symmetry factor
!----------------------------------------------------------------------------------
!  4. Error messages
!
!  5. Called by:
...
! 10. Source code
!------------------------------------------------------------------------------
integer(int4) :: iloc      ! counter
real(sp) :: dk13         ! distance between k1 and k3
real(sp) :: dk14         ! distance between k1 and k4
!------------------------------------------------------------------------------
!
...
! Type    I/O          Name             Description
!------------------------------------------------------------------------------
integer(int4), intent(in) ::  ik1     !    Index of k-component of wave number k1
integer(int4), intent(in) ::  ia1     !    Index of a-component of wave number k1
integer(int4), intent(in) ::  ik3     !    Index of k-component of wave number k3
integer(int4), intent(in) ::  ia3     !    Index of a-component of wave number k3
real(sp), intent(out)   ::  t13     !    Value of line integral over a specific locus
real(sp), intent(out)   ::  diagk1  !    Contribution to diagonal term of k1
real(sp), intent(out)   ::  diagk3  !    Contribution to diagonal term of k3
!
!  4. Error messages
!
!  5. Called by:
...
!     Local variables
!
integer(int4) :: iloc          ! counter along locus
integer(int4) :: ifnd           ! indicator if correct locus is found
integer(int4) :: ja2,ja2p      ! direction indices for interpolation of k2
integer(int4) :: jk2,jk2p      ! wave number indices for interpolation of k2
integer(int4) :: ja4,ja4p      ! direction indices for interpolation of k4
integer(int4) :: jk4,jk4p      ! wave number indices for interpolation of k4
integer(int4) :: ikq,iaq       ! counters
!
real(sp) :: sumt13           ! sum along locus
real(sp) :: qn1,qn2,qn3,qn4  ! action densities at wave numbers k1, k2, k3 and k4
real(sp) :: nprod            ! wave number product
real(sp) :: t2,t4            ! tail factors for k2 and k4
real(sp) :: qd1,qd3          ! contribution to diagonal term
real(sp) :: rterm            ! product term along locus
!
real(sp) :: qn13p            ! product of N1 and N3
real(sp) :: qn13d            ! difference of N1 and N3
!
integer(int4) :: jq_int    ! switch to activate test output
real(sp) :: wk,wa        ! weight for interpolation
real(sp) :: vk2,va2      ! wave number and direction for k2
real(sp) :: vk4,va4      ! wave number and direction for k4
!
!------------------------------------------------------------------------------
...
!     Local variables
!
integer(int4) :: iloc      ! counter along locus loop
integer(int4) :: jpos      ! index for interpolation and tracking of position in wave numebr array
integer(int4) :: itest     ! local test level
real(sp) :: k2a,k2m      ! angle (radians) and magnitude of wave number k2
real(sp) :: k4a,k4m      ! angle (radians) and magnitude of wave number k2
real(sp) :: dk           ! difference between two wave numbers
real(sp) :: xtest        ! test value for checking computation of weights, by inversion test
real(sp) :: ff,gg        ! variables in transformation of log-spacing to linear spacing
!
! functions used
!
...
!  3. Parameter used:
!
integer(int4), intent(in) :: npts      ! Number of points
real(sp), intent(in)    :: k1x       ! x-component of wave number k1
real(sp), intent(in)    :: k1y       ! y-component of wave number k1
real(sp), intent(in)    :: k3x       ! x-component of wave number k3
real(sp), intent(in)    :: k3y       ! y-component of wave number k3
!
real(sp), intent(out)   :: k2x(npts) ! x-component of wave number k2
real(sp), intent(out)   :: k2y(npts) ! y-component of wave number k2
real(sp), intent(out)   :: k4x(npts) ! x-component of wave number k4
real(sp), intent(out)   :: k4y(npts) ! y-component of wave number k4
real(sp), intent(out)   :: s(npts)   ! distance along locus
!
!  4. Error messages
!
!  5. Called by:
...
!     Local variables
!
integer(int4) :: ipt   ! counter of points along locus
!
real(sp) :: dirs     ! angle of symmetry axis
real(sp) :: dir1     ! direction of wave number k1
real(sp) :: dir3     ! direction of wave number k3
real(sp) :: dk0      ! step size along locus
real(sp) :: xk0      ! x-component
real(sp) :: yk0      ! y-component
real(sp) :: w2       ! radian frequency
real(sp) :: xx2,yy2  ! values along k2-locus
real(sp) :: xx4,yy4  ! values along k4-locus
real(sp) :: k1m      ! magnitude of wave number k1
!------------------------------------------------------------------------------
!
...
! Type    I/O          Name               Description
!------------------------------------------------------------------------------
integer(int4),intent(in)  :: nsig             ! number of radian frequencies
integer(int4),intent(in)  :: nang             ! number of directions
real(sp),   intent(in)  :: aspec(nsig,nang) ! Action density spectrum as a function of (sigma,theta)
real(sp),   intent(in)  :: sigma(nsig)      ! radian frequencies
real(sp),   intent(in)  :: angle(nang)      ! directions in radians (sector or full circle)
real(sp),   intent(in)  :: depth            ! water depth in m
real(sp),   intent(out) :: xnl(nsig,nang)   ! nonlinear quadruplet interaction computed with
!                                         a certain exact method (k,theta)
real(sp),   intent(out) :: diag(nsig,nang)  ! Diagonal term for WAM based implicit integration scheme
integer(int4), intent(out) :: ierror          ! error indicator
!
!  4. Error messages
!
!  5. Called by:
...
! local variables
!---------------------------------------------------------------------------------------
integer(int4) :: iaq      ! counter for directions
integer(int4) :: jaq      ! counter for directions
integer(int4) :: ikq      ! counter for wave numbers
integer(int4) :: iang     ! counter for directions
integer(int4) :: ia       ! counter for directions
integer(int4) :: ik       ! counter for wave numbers
integer(int4) :: idir1    ! direction in degrees of k1 (for integration test)
integer(int4) :: idir3    ! direction in degrees of k3 (for integration test)
real(sp) :: period      ! periodicity for direction, used in conversion of 2-spectra
real(sp) :: diagk1      ! diagonal term for k1
real(sp) :: diagk3      ! diagonal term for k3
!
real(sp) :: qn_max      ! maximum action density
real(sp) :: qn_min      ! minimum action density
!
!DIR$   ATTRIBUTES ALIGN : 64 :: cg
real(sp), dimension(nsig) :: cg         ! group velocity for conversion of spectrum and transfer
!
integer(int4) :: ia1,ia3,ja3   ! limits for directional loops
integer(int4) :: jk3           ! start of k3 loop
integer(int4) :: ik1,ik3       ! counters for wave number loop
integer(int4) :: nloc          ! number of points on locus
!
integer(int4) :: igrid         ! status of grid file
real(sp) :: t13              ! value of sub-integral
real(sp) :: k_rat            ! local ratio of wave numbers
real(sp) :: a_dif            ! directional difference
real(sp) :: jacobian         ! Jacobian
real(sp) :: qn1,qn3          ! action densities in k1 and k3
!
!  testing of diagonal term on a low level
!
real(sp) :: diagk1_0         ! saved value of diagk1
real(sp) :: diagk3_0         ! saved value of diagk3
real(sp) :: dq1              ! small change in action density of n1
real(sp) :: dq3              ! small change in action density of n3
real(sp) :: t13_0            ! Original estimated of diagonal term
real(sp) :: t13_1,t13_3      ! perturbed estimated of diagonal term
!
integer(int4) :: ifil_dir      ! indicator for filtering of directional criterion
integer(int4) :: ifil_krat     ! indicator for filtering of wave number ratio criterion
integer(int4) :: ifil_dens     ! indicator for filtering of action density criterion
integer(int4) :: ifil_tot      ! indicator for filtering due to any criterion
integer(int4) :: nfil_dir      ! counter to indicate filtering of directional criterion
integer(int4) :: nfil_krat     ! counter to indicate filtering of wave number criterion
integer(int4) :: nfil_dens     ! counter to indicate filtering of action density criterion
!
integer(int4) :: ntot_conf     ! total number of configurations
integer(int4) :: ntot_filt     ! total number of filtered configurations
!
integer(int4) :: icx1,icx3     ! test output for T13 test
real(sp) :: xt13             ! modified contribution of T13
!
!------------------------------------------------------------------------------
...
!  3. Parameters used:
!
real(sp), intent(in) :: k  ! wave number along symmetry axis of locus
!
!  4. Error messages
!
!  5. Called by:
...
!     Local variables
!
real(sp) :: qq    ! constant in direct polar method qq=q/sqrt(g)
real(sp) :: wk    ! intemediate radian frequency
real(sp) :: kz    ! intermediate wave number
!------------------------------------------------------------------------------
select case(iq_disp)
!
case(1)   ! deep water
...
real(sp), intent(in) ::       k1x     !  x-component of wave number k1
real(sp), intent(in) ::       k1y     !  y-component of wave number k1
real(sp), intent(in) ::       k2x     !  x-component of wave number k2
real(sp), intent(in) ::       k2y     !  y-component of wave number k2
real(sp), intent(in) ::       k3x     !  x-component of wave number k3
real(sp), intent(in) ::       k3y     !  y-component of wave number k3
real(sp), intent(in) ::       k4x     !  x-component of wave number k4
real(sp), intent(in) ::       k4y     !  y-component of wave number k4
integer(int4), intent(in) ::    iq_cple !  Type of coupling coefficient
real(sp), intent(in) ::       depth   !  Water depth in meters
real(sp), intent(in) ::       grav    !  Gravitational acceleration
!
!  4. Error messages
!
...
!Type    I/O         Name       Description
!-----------------------------------------------------------
real(sp), intent(in) ::  kxx      !  x-component of wave number
real(sp), intent(in) ::  kyy      !  y-component of wave number
!
!  4. Error messages
!
!
...
!     Local variables
!
real(sp) :: z               ! diferrence
real(sp) :: k2m,k4m         ! wave number magnitudes
real (dp) :: w2,w4  ! radian frequencies
!!real x_disper
!------------------------------------------------------------------------------
...
! Type    I/O        Name    Description
!--------------------------------------------------------------------
real(sp), intent(in) ::  x2   !  x-component of wave number k2
real(sp), intent(in) ::  y2   !  y-component of wave number k2
real(sp), intent(in) ::  x4   !  x-component of wave number k4
real(sp), intent(in) ::  y4   !  y-component of wave number k4
!
!  4. Error messages
!
!  5. Called by:
...
!
real(sp) :: k2m,k4m    ! wave number magnitudes
real(sp) :: k2md,k4md  ! k*d values
real(sp) :: ang2,ang4  ! directions
real(sp) :: cg2,cg4    ! group velocities
real(sp) :: sig2,sig4  ! radian frequencies
!------------------------------------------------------------------------------
k2m = sqrt(x2**2 + y2**2)
k4m = sqrt(x4**2 + y4**2)
!
...
! Type    I/O          Name       Description
!----------------------------------------------------------------------
real(sp), intent(in)   ::   k   !   wave number
real(sp), intent(in)   ::   d   !   water depth in m
!
!  4. Error messages
!
...
!     Local variables
!
integer(int4) :: id   ! copy of iq_type
real(sp) :: kd      ! k*d
!
kd = k * d
id = iq_disp
...
!     Local variables
!
real(sp) :: k4       ! wave number magnitudes of k4
real(sp) :: w2,w4    ! radian frequencies of wave numbers k2 and k4
real(sp) :: z        ! function value
!
!!real x_disper
!
select case(iq_disp)
...
!     local variables
!
real(sp) :: kk2x,kk2y,kk2m  ! wave number components and magnitude for k2
real(sp) :: kk4x,kk4y,kk4m  ! wave number components and magnitude for k4
real(sp) :: w2,w4           ! radian frequencies of wave numbers k2 and k4
real(sp) :: z               ! function value
!!real x_disper
...
implicit none
!
real(sp) :: z4x,z4y  ! dummy arguments
!
real(sp) :: w1x0,w1y0,w2x0,w2y0,w3x0,w3y0,h,dsq
real(sp) :: om1,om2,om3,om4,scpl1,scpl2,scpl3,stot
real(sp) :: t1,t2,t3,t4,t5,tot1,tot2,tot3,tot4,tot5
real(sp) :: som1,som2,som3
real(sp) :: s1,s2,s3,z1,z2,z3,z4,z5
real(sp) :: p1,p2,p3,p4,di,tnz1,tnz2,tnz3,tnz23
real(sp) :: csz1,csz2,csz3,csz23
real(sp) :: e,g,gsq,omsq23,pi4
real(sp) :: dot123,dot23
!!real cosz,tanz
!
real(sp) :: eps
!
...
!
!
real(dp) :: wsqp12         ! derived variable
real(dp) :: wsqm13         ! derived variable
real(dp) :: wsq13          ! derived variable
real(dp) :: wsqm14         ! derived variable
real(dp) :: wsq14          ! derived variable
real(dp) :: wsq12          ! derived variable
real(sp) :: z,z12,z13,z14              ! derived variables
real(sp) :: dwebb                      ! final coefficient
real(sp) :: p1,p2,p3,p4,p5,p6,p7,p8,p9 ! partial summations
real(sp) :: w1,w2,w3,w4                ! radian frequencies
real(sp) :: k1,k2,k3,k4                ! wave number magnitudes
real(sp) :: dot12                      ! k1*k2
real(sp) :: dot13                      ! k1*k3
real(sp) :: dot14                      ! k1*k4
real(sp) :: dot23                      ! k2*k3
