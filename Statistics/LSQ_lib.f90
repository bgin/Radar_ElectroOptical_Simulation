...
!23.  PROG7.F90        
      ...
      SUBROUTINE BNDACC (G,MDG,NB,IP,IR,MT,JT)  !GCC$ attributes hot :: BNDACC !GCC$ attributes aligned(32) :: BNDAC 
         use omp_lib
#if 0
!c
...
!C                                             ALG. STEP 25  
      ...
      SUBROUTINE BNDSOL (MODE,G,MDG,NB,IP,IR,X,N,RNORM) !GCC$ ATTRIBUTES HOT :: BNDSOL !GCC$ ATTRIBUTES ALIGNED(32) :: BNDSOL 
         use omp_lib  
#if 0  
!c
...
!$OMP SIMD REDUCTION(+:RSQ) LINEAR(J:1)
           DO 30 J=NP1,IRM1 
...
!C                                   ALG. STEP 30
           IE=min(N+1-I,NB)
!$OMP SIMD REDUCTION(+:S) LINEAR(J:1)
                DO 60 J=2,IE
                JG=J+L  
                IX=I-1+J
   60           S=S+G(I,JG)*X(IX)   
...
!C   
  130 write (*,'(/a/a,4i6)')' ZERO DIAGONAL TERM IN BNDSOL.', &
      ...
      END
       ...
       double precision FUNCTION DIFF(X,Y) !GCC$ ATTRIBUTES INLINE :: DIFF !GCC$ ATTRIBUTES PURE :: DIFF !GCC$ ATTRIBUTES ALIGNED(64) :: DIFF 
        use omp_lib
...
!C
   ...
   SUBROUTINE G1 (A,B,CTERM,STERM,SIG)   
      use omp_lib
!$OMP DECLARE SIMD UNIFORM(CTERM,STERM,SIG)
#if 0
!c
...
!   ELEMENTAL   SUBROUTINE G2    (CTERM,STERM,X,Y) !GCC$ ATTRIBUTES INLINE :: G2 !GCC$ ATTRIBUTES ALIGNED(32) :: G2 
!c
...
!C     IF INITIAL I = 5        THE PERIOD WILL BE 10 
   30 I=I*MI
    ...
    SUBROUTINE H12 (MODE,LPIVOT,L1,M,U,IUE,UP,C,ICE,ICV,NCV)  !gCC$ ATTRIBUTES HOT :: H12 !GCC$ ATTRIBUTES ALIGNED(32) :: H12
      use omp_lib
!C   
...
!$OMP SIMD REDUCTION(+:SM)  LINEAR(J:1)
          DO 30 J=L1,M  
...
!C   
   ...
   80 B=ONE/B   
      ...
      INCR=ICE*(L1-LPIVOT)  
          ...
          I4=I3 
          SM=C(I2)*UP
!$OMP SIMD REDUCTION(+:SM)  LINEAR(I:1)
              DO 90 I=L1,M  
              SM=SM+C(I3)*U(1,I)
   90         I3=I3+ICE 
          IF (SM) 100,120,100   
...
!$OMP SIMD  LINEAR(I:1)
  ...
  130 RETURN
      ...
      SUBROUTINE HFTI (A,MDA,M,N,B,MDB,NB,TAU,KRANK,RNORM,H,G,IP)   !GCC$ ATTRIBUTES HOT :: HFTI !GCC$ ATTRIBUTES ALIGNED(32) :: HFTI
          use omp_lib
!c!
...
!$OMP SIMD REDUCTION(+:TMP) LINEAR(I:1)
              DO 120 I=KP1,M
...
!$OMP SIMD REDUCTION(+:SM) 
  ...
  270 KRANK=K   
      ...
      SUBROUTINE LDP (G,MDG,M,N,H,X,XNORM,W,INDEX,MODE)    !GCC$ ATTRIBUTES ALIGNED(32) :: LDP !GCC$ ATTRIBUTES HOT :: LDP 
        use omp_lib
!c
...
!C                      USE THE FOLLOWING RETURN IF UNSUCCESSFUL IN NNLS.
   ...
   50 FAC=ONE   
      IW=IY-1   
!$OMP SIMD REDUCTION(+:FAC)  LINEAR(I:1)
          DO 60 I=1,M   
          IW=IW+1   
...
!C                             RETURNING WITH CONSTRAINTS NOT COMPATIBLE.
  130 MODE=4
      ...
      subroutine MFEOUT (A, MDA, M, N, NAMES, MODE, UNIT, WIDTH) !GCC$ ATTRIBUTES ALIGNED(32) :: MFEOUT !GCC$ ATTRIBUTES COLD :: MFEOUT
#if 0
c
...
!C   
      ...
      UNORM=ZERO
      IF (NSETP .ne. 0) then
!$OMP SIMD REDUCTION(+:UNORM)  LINEAR(L:1)
          DO 90 L=1,NSETP   
...
!$OMP SIMD ALIGNED(ZZ:64,B:64) LINEAR(L:1)  
         DO 120 L=1,M  
  120        ZZ(L)=B(L)    
...
C     ------------------------------------------------------------------   
      ...
      SUBROUTINE QRBD (IPASS,Q,E,NN,V,MDV,NRV,C,MDC,NCC)   !GCC$ ATTRIBUTES HOT :: QRBD  
         use omp_lib
...
!C   
  ...
  270      CONTINUE     
      ...
      RETURN
      END   
      subroutine SVA(A,MDA,M,N,MDATA,B,SING,KPVEC,NAMES,ISCALE,D,WORK) !GCC$ ATTRIBUTES HOT :: SVA
         use omp_lib
#if 0
c
...
!C
         DO 52 J = 1,N
            ...
            if( ISCALE .le. 2) then
               SB = ZERO
!$OMP SIMD REDUCTION(+:SB)  LINEAR(I:1)
               DO 30 I = 1,M
   30             SB = SB + A(I,J)**2
               A1 = sqrt(SB)
...
!$OMP SIMD REDUCTION(+:SB) LINEAR(I:1)
   ...
   70 CONTINUE
      WORK(MINMN+1) = SB
!$OMP SIMD REDUCTION(+:SB)  LINEAR(J:1)
      DO  75 J = MINMN, 1, -1
         SB = SB + B(J)**2
         WORK(J) = SB
   75 CONTINUE
...
!C                                    COMPUTE        ALAMB = 10.0**EL
            ...
            YS = ZERO
            RS = WORK(NSOL+1)
!$OMP SIMD REDUCTION(+:YS,RS)  LINEAR(I:1)
            DO 190 I = 1,MINMN
               SL = SING(I)**2 + ALAMB**2
...
c     ------------------------------------------------------------------
#endif
      subroutine SVDRS (A, MDA, M1, N1, B, MDB, NB, S, WORK) !GCC$ ATTRIBUTES HOT :: SVDRS !GCC$ ATTRIBUTES ALIGNED(32) :: SVDRS
