...
INTEGER(i4), SAVE                :: nobs, ncol, r_dim
INTEGER(i4), ALLOCATABLE, SAVE   :: vorder(:), row_ptr(:)
LOGICAL(i4), SAVE                :: initialized = .false.,                  &
...
SUBROUTINE sing(lindep, ifault)
 !dir$ optimize:3
 !dir$ attributes code_align : 32 :: sing
 !dir$ attributes forceinline :: sign
!     ALGORITHM AS274  APPL. STATIST. (1992) VOL.41, NO. 2
...
SUBROUTINE ss()
  !dir$ optimize:3
  !dir$ attributes code_align : 32 :: ss
  !dir$ attributes forceinline :: ss
  use omp_lib
!     ALGORITHM AS274  APPL. STATIST. (1992) VOL.41, NO. 2
...
total = sserr
rss(ncol) = sserr
!dir$ assume_aligned d:64,rhs:64,rss:64
!dir$ vector aligned
...
SUBROUTINE cov(nreq, var, covmat, dimcov, sterr, ifault)
  !dir$ optimize:3
  !dir$ attributes code_align : 32 :: cov
  use omp_lib
!     ALGORITHM AS274  APPL. STATIST. (1992) VOL.41, NO. 2
...
DO row = 1, nreq
  ...
  DO col = row, nreq
    ...
    END IF
    !dir$ assume_aligned rinv:64,d:64
    !dir$ vector aligned
    !$omp simd reduction(+:total)
    DO K = col+1, nreq
...
SUBROUTINE inv(nreq, rinv)
  !dir$ optimize:3
  !dir$ attributes code_align : 32 :: inv
  !dir$ attributes forceinline :: inv
  use omp_lib
!     ALGORITHM AS274  APPL. STATIST. (1992) VOL.41, NO. 2
...
DO row = nreq-1, 1, -1
  ...
  DO col = nreq, row+1, -1
    ...
    pos2 = pos
    total = zero
    !dir$ assume_aligned r:64,rinv:64
    !dir$ vector aligned
    !$omp simd reduction(+:total)
    DO k = row+1, col-1
      pos2 = pos2 + nreq - k
...
SUBROUTINE partial_corr(in, cormat, dimc, ycorr, ifault)
  !dir$ optimize:3
  !dir$ attributes code_align : 32 :: partial_corr
  use omp_lib
...
!dir$ attributes align : 64 :: rms,work
...
DO col = in+2, ncol
  pos = base_pos + col
  sumxx = d(col)
  !dir$ assume_aligned d:64,r:64
  !dir$ vector aligned
  !$omp simd reduction(+:sumxx)
  DO row = in+1, col-1
...
DO col1 = in+1, ncol
  ...
  DO row = in+1, col1-1
    pos2 = pos1 + 1
    !dir$ assume_aligned work:64,d:64,r:64
    !dir$ vector aligned
    !$omp simd reduction(+:work)
    DO col2 = col1+1, ncol
...
SUBROUTINE vmove(from, to, ifault)
   !dir$ optimize:3
   !dir$ attributes code_align : 32 :: vmove
...
SUBROUTINE hdiag(xrow, nreq, hii, ifault)
  !dir$ optimize:3
  !dir$ attributes code_align : 32 :: hdiag
  !dir$ attributes forceinline :: hdiag
  use omp_lib
!     ALGORITHM AS274  APPL. STATIST. (1992) VOL.41, NO. 2
!
!                         -1           -1
...
FUNCTION varprd(x, nreq) RESULT(fn_val)
  !dir$ optimize:3
  !dir$ attributes forceinline :: varprd
  !dir$ attributes code_align : 32 :: varprd
  use omp_lib
...
!dir$ assume_aligned wk:64,d:64
!dir$ vector aligned
!$omp simd reduction(+:fn_val)
DO row = 1, nreq
...
SUBROUTINE bksub2(x, b, nreq)
  !dir$ optimize:3
  !dir$ attributes forceinline :: bksub2
  !dir$ attributes code_align : 32 :: bksub2
  use omp_lib
