...
!      character(80) measFile    !name of file containing measurement data
      ...
      real(kind=sp) :: tepoch           !epoch time of x0 and p0
      real(kind=sp) :: tend             !end time for filter processing
      real(kind=sp), dimension(3) :: rsum          !residual sum-of-squares for 3 filters

      real(kind=sp), dimension(:),  allocatable   :: xfu
      real(kind=sp), dimension(:),  allocatable   :: ud
      real(kind=sp), dimension(:),  allocatable   :: xfk
      real(kind=sp), dimension(:,:),allocatable   :: pf
      real(kind=sp), dimension(:),  allocatable   :: xfs
      real(kind=sp), dimension(:),  allocatable   :: xfa
      real(kind=sp), dimension(:,:),allocatable   :: pfa
      real(kind=sp), dimension(:),  allocatable   :: rsrif
      real(kind=sp), dimension(:,:),allocatable   :: dxsum
      real(kind=sp), dimension(:,:),allocatable   :: sigsum
...
!      real(kind=dp) rnoise(mmaxi,mmaxi)           !meas noise covariance
      ...
      type, public :: states_soa
          ...
          integer(i4)   :: status      !0=ignore, 1=unadjusted analyze, 2=consider, 3=estimate
          integer(i4)   :: xtype       !1=core state, 2=1 or 2 order Markov, 3=bias
          real(kind=sp), dimension(:),   allocatable :: x0    !a priori estimate at epoch time
          real(kind=sp), dimension(:),   allocatable :: sig0  !1-sigma uncertainty of x0 at epoch time
          real(kind=sp), dimension(:),   allocatable :: qs    !PSD of driving white noise
          real(kind=sp), dimension(:,:), allocatable :: const !known constants associated with this state in dynamic or measurement model
      ...
      type, public :: measures_soa
          ...
          integer(i4)   :: mtype       !meas type (must match types in model)
          integer(i4)   :: m           !number of scalar meas included in this meas
          real(kind=sp), dimension(:),   allocatable :: t    ! measures time
          real(kind=sp), dimension(:,:), allocatable :: y    ! mmax scalar values per single measure
          real(kind=sp), dimension(:,:), allocatable :: sigr ! !1-sigma meas noise for mmax scalar measures
