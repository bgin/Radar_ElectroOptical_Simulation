...
void gpu_int32_memset(int32_t * __restrict d_ptr,
                      int32_t value,
                      size_t count,
                      int32_t * __restrict ierr) {
       if(__builtin_expect(*ierr < 0,1)) *ierr = 0;
       if(__builtin_expect(0 >= count,0)) {
		*ierr = -1;
		return;
	}
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(int32_t));
#else
        GMS_CUDA_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(int32_t));
#endif
        *ierr = 0;
        return;
Error:
...
void gpu_float_memset(float* __restrict d_ptr,
                      int32_t value,
                      size_t count,
                      int32_t * __restrict ierr) {
       if(__builtin_expect(*ierr < 0,1)) *ierr = 0;
	...
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(float));
#else
        GMS_CUDA_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(float));
#endif
        *ierr = 0;
        return;
Error:
...
void gpu_int32_memset(double * __restrict d_ptr,
                      int32_t value,
                      size_t count,
                      int32_t * __restrict ierr) {
       if(__builtin_expect(*ierr < 0,1)) *ierr = 0;
	...
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(double));
#else
        GMS_CUDA_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(double));
#endif
        *ierr = 0;
        return;
Error:
...
void gpu_complex4_memset(cuComplex * __restrict d_ptr,
                         int32_t value,
                         size_t count,
                         int32_t * __restrict ierr) {
       if(__builtin_expect(*ierr < 0,1)) *ierr = 0;
	...
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(cuComplex));
#else
        GMS_CUDA_CHECK(cudaMemset((void*)&d_ptr[0],value,count*sizeof(cuComplex));
#endif
        *ierr = 0;
        return;
Error:
...
void host_alloc_int32(int32_t * __restrict h_ptr,
                      size_t size,
                      uint32_t flags,
                      int32_t * __restrict ierr) {
	...
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(int32_t),flags));
#else
        GMS_CUDA_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(int32_t),flags));
#endif
       *ierr = 0;
        return;
...
Error:
	  ...
	  fatal_gpu_error(__PRETTY_FUNCTION__,
	   ...
	   if(d_ptr) cudaFreeHost((void*)&h_ptr[0]);
	      *ierr = -2;
             return;
       }  
}
...
void host_alloc_float(float * __restrict h_ptr,
                      size_t size,
                      uint32_t flags,
                      int32_t * __restrict ierr) {
	...
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(float),flags));
#else
        GMS_CUDA_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(float),flags));
#endif
       *ierr = 0;
        return;
...
void host_alloc_double(double * __restrict h_ptr,
                      size_t size,
                      uint32_t flags,
                      int32_t * __restrict ierr) {
	...
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(double),flags));
#else
        GMS_CUDA_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(double),flags));
#endif
       *ierr = 0;
        return;
...
void host_alloc_complex4(cuComplex * __restrict h_ptr,
                         size_t size,
                         uint32_t flags,
                         int32_t * __restrict ierr) {
	...
	cudaError_t status;
#if (GMS_CUDA_DEBUG_ON) == 1
        GMS_CUDA_DEBUG_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(cuComplex),flags));
#else
        GMS_CUDA_CHECK(cudaHostAlloc((void**)&h_ptr,size*sizeof(cuComplex),flags));
#endif
       *ierr = 0;
        return;
