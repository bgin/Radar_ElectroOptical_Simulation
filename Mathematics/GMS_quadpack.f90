...
!  * Jacob Williams, Jan 2022 : modernized this procedure. added quad-precision coefficients.
    ...
    subroutine savint(x,y,n,xlo,xup,ans,ierr)

    implicit none

    real(kind=sp),dimension(:),intent(in) :: x !! array of abscissas, which must be in increasing order.
    real(kind=sp),dimension(:),intent(in) :: y !! array of function values. i.e., `y(i)=func(x(i))`
    integer(kind=i4),intent(in) :: n !! The integer(kind=i4) number of function values supplied.
                            !! `N >= 2` unless `XLO = XUP`.
    real(kind=sp),intent(in) :: xlo !! lower limit of integration
    real(kind=sp),intent(in) :: xup !! upper limit of integration.  Must have `XLO <= XUP`
    real(kind=sp),intent(out) :: ans !! computed approximate value of integral
    integer(kind=i4),intent(out) :: ierr !! A status code:
                                !!
                                !! * Normal Code
                                !!    * =1 Means the requested integration was performed.
    ...
    integer(kind=i4) :: i , inlft , inrt , istart , istop
    real(kind=sp) :: a , b , c , ca , cb , cc , fl , fr , r3 , &
                rp5 , slope , sum , syl , syl2 , syl3 , syu , &
                syu2 , syu3 , term1 , term2 , term3 , x1 , &
                x12 , x13 , x2 , x23 , x3
