...
!
  ...
  integer ( kind = i4 ), parameter :: mxstge = 30
  integer ( kind = i4 ), parameter :: maxtbl = 10
  integer ( kind = i4 ), parameter :: maxts = 2049
  ...
  integer ( kind = i4 ) ibegs(mxstge)
  integer ( kind = i4 ) iend
  integer ( kind = i4 ) ii
  integer ( kind = i4 ) iii
  integer ( kind = i4 ) ind
  integer ( kind = i4 ) istage
  integer ( kind = i4 ) istep
  integer ( kind = i4 ) istep2
  integer ( kind = i4 ) it
  integer ( kind = i4 ) l
  integer ( kind = i4 ) lm1
  integer ( kind = i4 ) n
  integer ( kind = i4 ) n2
  integer ( kind = i4 ) nnleft
  real ( kind = dp ) prever
  ...
  begin(1:mxstge) = 0.0_dp
  est(1:mxstge) = 0.0_dp
  finis(1:mxstge) = 0.0_dp
  ibegs(1:mxstge) = 0
  reglsv(1:mxstge) = .false.
 
  vint = 0.0_dp
 
  rn(1:4) = (/ 0.7142005_dp, 0.3466282_dp, 0.8437510_dp, 0.1263305_dp /)
  ...
  ind = 1
  bma = abs ( b - a )
  errr = min ( 0.1_dp, max ( abs ( relerr ), 10.0_dp*rnderr) )
  erra = abs ( abserr )
  ...
  reglar = .false.
  beg = a
  fbeg = func ( beg ) / 2.0_dp
  ts(1) = fbeg
  ibeg = 1
  end = b
  fend = func ( end ) / 2.0_dp
  ts(2) = fend
  iend = 2
...
60 continue
  ...
  t(l,1) = t(l-1,1) *0.5_dp + sum1 / fn
  tabs = tabs *0.5_dp + sumabs / fn
  ...
  fextrp = 1.0D+00
  do i = 1, lm1
    fextrp = fextrp * 4.0_dp
    t(i,l) = t(l,i) - t(l-1,i)
    t(l,i+1) = t(l,i) + t(i,l) / ( fextrp - 1.0_dp )
  end do
...
90 continue
  ...
  if ( abs ( 4.0_dp - t(1,lm1) ) <= h2tol ) then
    go to 130
  end if

  if ( t(1,lm1) == 0.0_dp ) then
    go to 120
  end if

  if ( abs ( 2.0_dp - abs ( t(1,lm1) ) ) < tljump ) then
    go to 280
  end if
...
150 continue
  ...
  it = it + 1
  vint = step * t(l,it)
  errer = abs ( step / ( fextrp - 1.0_dp ) * t(it-1,l))
  ...
  if ( abs ( t(it,lm1) *0.25_dp - fextrp ) / fextrp < aittol ) then
    fextrp = fextrp * 4.0_dp
  end if
...
210 continue
  ...
  fextrp = singnx
  singnx = 2.0_dp * singnx
  go to 240

230 continue

  fextrp = h2next
  h2next = 4.0_dp * h2next
...
270 continue
  ...
  if ( 2.0_dp < l - it .and. istage < mxstge ) then
    go to 370
  end if
...
280 continue
  ...
  diff = abs ( t(1,l) ) * 2.0_dp* fn
  go to 340
 
290 continue

  slope = ( fend - fbeg ) * 2.0_dp
  fbeg2 = fbeg * 2.0_dp
...
!
  ...
  integer ( kind = i4 ), parameter :: nupper = 9
  ...
  integer ( kind = i4 ) j
  integer ( kind = i4 ) k
  integer ( kind = i4 ) ksign
  integer ( kind = i4 ) n
  integer ( kind = i4 ) ncof
  integer ( kind = i4 ) nhalf
  integer ( kind = i4 ) nn
...
!
  const1 = 0.5_dp * ( func ( a ) + func ( b ) )
  const2 = func ( bet )
  acof(1) = 0.5_dp * ( const1 + const2 )
  acof(2) = 0.5_dp * ( const1 - const2 )
  bcof(2) = acof(2)
  tend = 2.0_dp * ( acof(1) - acof(2) / 3.0_dp )
!
...
!
    const1 = -sin ( triarg )
    const2 = 0.5_dp * alfno / const1
    alfno = const1
    betno = const2
    gamman = 1.0_dp - 2.0_dp * alfno**2
    deltan = -2.0_dp * alfno * betno
    bcof(1) = 0.0_dp
...
!
      ...
      cof = 2.0_dp * ( 2.0_dp * etank**2 - 1.0_dp )
      a2 = 0.0_dp
      a1 = 0.0_dp
      a0 = ccof(nhalf)
...
!
    ...
    do j = 1, nhalf
      index = nn - j
      ccof(j) = 0.5_dp * ( acof(j) + bcof(j) )
      ccof(index) = 0.5_dp * ( acof(j) - bcof(j) )
      const1 = abs ( ccof(j) )
...
!
    wmean = 0.5_dp * ( tend + umid )
    bounds = 0.5_dp * ( tend - umid )
    deln = 0.0_dp
    rk = 2.0_dp * rn
    do j = 1, nhalf
      index = n + 2 - j
...
160 continue
    ...
    n = 2 * n
    rn = 2.0_dp * rn
    hnstep = 0.5_dp * hnstep
    triarg = 0.5_dp * triarg
