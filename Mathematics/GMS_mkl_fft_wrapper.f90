...
module mkl_fft_wrappers
 ...
 !==================================================================================85
  ...
  use IFPORT,       only :  TRACEBACKQQ
    ...
    ! Major version
    integer(i4), parameter, public :: MKL_FFT_WRAPPER_MAJOR = 1
    ...
    subroutine fft_threaded_cipf(datum,data_len1,data_len2,dim_len,nthreads,status)
          ...
          !$OMP  PARALLEL DO SCHEDULE(STATIC,1) SHARED(length,datum) PRIVATE(xth,handle,status)
            do xth = 1, nthreads
    ...
    subroutine fft_threaded_cipb(datum,data_len1,data_len2,dim_len,nthreads,status)
         ...
         !DIR$ ASSUME_ALIGNED datum:64
          ...
          integer( i4), dimension(2) :: length = [data_len1,data_len2]
          ! Executable statemetns
          !$OMP PARALLEL DO SCHEDULE(STATIC,4) SHARED(length,datum) PRIVATE(xth,handle,status)
            do xth = 1, nthreads
                status = DftiCreateDescriptor(handle,DFTI_DOUBLE,DFTI_COMPLEX,dim_len,length)
                status = DftiCommitDescriptor(handle)
