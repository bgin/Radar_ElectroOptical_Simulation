...
!   * Bernard Gingold : 29/12/2022 , minor changes added.
...
!  is returned instead.
    ...
    integer(kind=i4),parameter :: inf = -minexponent(1.0_dp) - 2 !! the largest possible positive integers subject to
                                                        !! 2**(-inf) and -2**(-inf) being representable real numbers.
    integer(kind=i4),parameter :: sup = maxexponent(1.0_dp) - 1  !! the largest possible positive integers subject to
                                                        !! 2**sup and -2**sup being representable real numbers.
    ...
    if (iord<1 .or. iord>3 .or. xmax<=xmin .or. &
      ...
      ifail = 2
        ...
        twoinf = 2.0_dp**(-inf)
        twosup = 2.0_dp**sup
        factor = 2.0_dp**(real((inf+sup),dp)/30.0_dp)
        ...
        if (abs(h1) > twosup/128.0_dp) then
          hacc1 = twosup
        else
          hacc1 = 128.0_dp*h1
        end if

        if (abs(hacc1)/4.0_dp < minh) then
        ...
        do
            ...
            ! initialization of steplengths, accuracy and other parameters
              ...
              if (method == 2) newacc = (acc1+acc2)/2.0_dp
            ...
            ! update estimated accuracy of function values
              ...
              if (method /= -1 .and. abs(nhacc1) <= 1.125_dp*abs(heval)/beta4) then
                ...
                if (ifail==-1) return
                if (facc1 < pacc1) facc1=(3.0_dp*facc1+pacc1)/4.0_dp
              end if
              if (method /= 1 .and. abs(nhacc2) <= 1.125_dp*abs(heval)/beta4) then
                ...
                if (ifail==-1) return
                if (facc2 < pacc2) facc2 = (3.0_dp*facc2+pacc2)/4.0_dp
              end if
              if (method == 1 .and. newacc < facc1) newacc = facc1
              if (method == -1 .and. newacc < facc2) newacc = facc2
              if (method == 2 .and. newacc < (facc1+facc2)/2.0_dp) &
                      newacc = (facc1+facc2)/2.0_dp
            end if
            ...
            do k = 1,n
              ...
              if (.not. ignore(k)) then
                ...
                if (minerr(k) < twosup/4.0_dp) then
                  s = 4.0_dp*minerr(k)
                else
                  s = twosup
                end if
...
! smallest spacing at which the function values are unequal near `x0+h0`.
    ...
    integer(kind=i4), intent(out)     :: ifail  !! 0 if no error, -1 if user termination.

    real(kind=dp) :: a0,a1,f00,f001,f2,deltaf,t0,t1,df(5)
    integer(kind=i4) :: j
    ...
    if (h0 /= 0.0_dp) then
      ...
      if (abs(h1) >= 32.0_dp*twoinf) h1 = h1/8.0_dp
      if (16.0_dp*abs(h1) > abs(h0)) h1 = sign(h1,1.0_dp)*abs(h0)/16.0_dp
      f001 = me%f(x0+h0-h1)
      if (me%stop) then
        ifail = -1
        return
      ...
      if (f001 == f00) then
        if (256.0_dp*abs(h1) <= abs(h0)) then
          ...
          do
              ...
              if (f001 /= f00 .or. 256.0_dp*abs(h1) > abs(h0)) exit
              h1 = 2.0_dp*h1
          end do
          h1 = 8.0_dp*h1
        else
          h1 = sign(h1,1.0_dp)*abs(h0)/16.0_dp
        end if
      else
        if (256.0_dp*twoinf <= abs(h0)) then
          do
              f001 = me%f(x0+h0-h1/2.0_dp)
              if (me%stop) then
                ifail = -1
                return
              end if
              if (f001 == f00 .or. abs(h1) < 4.0_dp*twoinf) exit
              h1 = h1/2.0_dp
          end do
          h1 = 8.0_dp*h1
          if (16.0_dp*abs(h1) > abs(h0)) h1 = sign(h1,1.0_dp)*abs(h0)/16.0_dp
    ...
    do j = 1,5
      f2 = me%f(x0+h0-real(2*j-1,dp)*h1)
      if (me%stop) then
        ifail = -1
        return
      end if
