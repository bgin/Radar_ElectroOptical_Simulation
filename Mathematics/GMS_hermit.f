...
#endif
      EXTERNAL FUNSUB
      INTEGER(kind=i4) :: NDIM, NF, MINPTS, MAXPTS, RESTAR
      INTEGER(kind=i4) :: NEVAL, IFAIL
      REAL(kind=dp)    :: EPSABS, EPSREL
      REAL(kind=dp) RESULT(*), ABSERR(*), WORK(*)
!*
!*   Local variables.
!*
      INTEGER(kind=i4) I, MAXRUL, RULE, MNRULE, INTCLS, NUMSMS
      PARAMETER ( MAXRUL = 25 )
      REAL(kind=dp) DIFFER, CLTOTL, RNTCLS, EPOWER
...
!*
      ...
      ELSE
         ...
         DO WHILE ( NEVAL .LE. MAXPTS .AND. RULE .LE. MAXRUL                &
            ...
            RNTCLS = REAL( INTCLS,kind=dp )**EPOWER 
...
******INPUT PARAMETERS
*  S       INTEGER(kind=i4) number of variables, with 0 < S <= 1000.
...
*  MINORD  INTEGER(kind=i4) minimum order parameter.  On entry MINORD specifies
...
*  MAXORD  INTEGER(kind=i4) maximum order parameter, must be greater than MINORD
...
*          degree 2*MAXORD+1.
*  INTCLS  INTEGER(kind=i4) number of F values needed for INTVAL
...
*  NUMSMS  INTEGER(kind=i4) number of S-partitions of the integers 0,...,MAXORD.
************************************************************************
#endif

      EXTERNAL F
      INTEGER(kind=i4) S, N, MAXDIM, MINORD, MAXORD, MAXRDM, NUMSMS
      PARAMETER ( MAXDIM = 1000, MAXRDM = 25 ) 
      REAL(kind=dp) X(MAXDIM), FULWGT, WEIGHT,                              &
                      INTVAL(N), FULSMS(N+1,*), WORK(*) 
      INTEGER(kind=i4) D, I, L, MODOFM, M(MAXDIM), K(MAXDIM), INTCLS, PRTCNT
      D = MINORD + 1
      INTCLS = 0
      IF ( D .EQ. 0 ) THEN
         DO I = 1,N
...
!*
      SUBROUTINE FULSMH( S, M, N, F, FULSMS, X, FUNVAL )
           ...
           !DIR$ ATTRIBUTES INLINE :: FULSMH
           !DIR$ ATTRIBUTES VECTOR:PROCESSOR(skylake_avx512) :: FULSMH
           !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: FULSMH
#endif
!*
!****  To compute fully symmetric basic rule sums
!*
      INTEGER(kind=i4) S, M(*), SUMCLS, IX, LX, I,L, MI, ML, IL, N, MX
      REAL(kind=dp) X(*), INTWGT, FULSMS(*), FUNVAL(*)
!*
...
!*
      ...
      REAL(kind=dp) G(0:MX)
      SAVE G
      DATA G( 0),G( 1)/0.0_dp               , 0.17320508075688773D1/
...
!*
      ...
      !dir$ assume_aligned FULSMS:64
      !dir$ assume_aligned FUNVAL:64
#endif
 10   DO I = 1, S
         X(I) = -G(M(I))
      END DO
...
!*
      ...
      REAL(kind=dp) FUNCTION WEIGHT( S, INTRPS, M,K, MODOFM, D )
               use mod_kinds, only : i4,dp
#if defined (__INTEL_COMPILER)
           !DIR$ ATTRIBUTES INLINE :: WEIGHT
           !DIR$ ATTRIBUTES VECTOR:PROCESSOR(skylake_avx512) :: WEIGHT
           !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: WEIGHT
#endif
!*
!****  Subroutine to update weight for partition m
!*
      INTEGER(kind=i4) S, M(S), K(S), I, L, D, MAXRDM, NZRMAX, MODOFM
      PARAMETER ( MAXRDM = 25 )
      REAL(kind=dp) INTRPS(S), MOMPRD(0:MAXRDM,0:MAXRDM), MOMNKN,           &
                    INTMPA, INTMPB
                  
      SAVE MOMPRD
#if defined (_OPENMP)
      !$OMP THREADPRIVATE(MOMPRD)
#endif
!*
...
!*
      PARAMETER ( NZRMAX = 17 )
      REAL(kind=dp) A(0:NZRMAX), G(0:NZRMAX)
...
!*
 ...
 10   INTRPS(1) = MOMPRD( M(1), K(1) )
      ...
      SUBROUTINE NXPART( PRTCNT, S, M, MODOFM )
          use mod_kinds, only : i4,dp
#if defined (__INTEL_COMPILER)
           !DIR$ ATTRIBUTES INLINE :: NXPART
           !DIR$ ATTRIBUTES VECTOR:PROCESSOR(skylake_avx512) :: NXPART
           !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: NXPART
#endif
!*
!****  SUBROUTINE TO COMPUTE THE NEXT S PARTITION
!*
      INTEGER(kind=i4) S, M(S), PRTCNT, MODOFM, I, MSUM, L
      IF ( PRTCNT .EQ. 0 ) THEN
         DO I = 1, S
            M(I) = 0
         END DO
      ...
      SUBROUTINE MLTRUL( NDIM, NUMFUN, FUNSUB, NP, POINT, WEIGHT,           &
                 ...
                 use mod_kinds, only : i4,dp
#if defined (__INTEL_COMPILER)
           !DIR$ ATTRIBUTES INLINE :: MLTRUL
           !DIR$ ATTRIBUTES VECTOR:PROCESSOR(skylake_avx512) :: MLTRUL
           !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: MLTRUL
#endif                 
!*
!*     Computes product integration rule
!*
      EXTERNAL FUNSUB
      INTEGER(kind=i4) NDIM, NP, NUMFUN, ICI, I
      REAL(kind=dp) POINT(*), WEIGHT(*), INTVAL(*)
      REAL(kind=dp) WTPROD, X(*), FUNS(*), IC(*)
      DO I = 1, NDIM
         IC(I) = 1
      END DO
      DO I = 1, NUMFUN
 ...
 10   WTPROD = 1
      ...
      SUBROUTINE HERMIT( NDIM, NUMFUN, MINPTS, MAXPTS, FUNSUB,              &
             ...
             use mod_kinds, only : i4,dp
#if defined (__INTEL_COMPILER)
           !DIR$ ATTRIBUTES INLINE :: HERMIT
           !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: HERMIT
#endif 
#if 0                 
...
#endif
      EXTERNAL FUNSUB
      INTEGER(kind=i4) NDIM, NUMFUN, MINPTS, MAXPTS, RESTAR
      INTEGER(kind=i4) NEVAL, IFAIL
      REAL(kind=dp) EPSABS, EPSREL
      REAL(kind=dp) RESULT(*), ABSERR(*), WORK(*)
!*
!*   Local variables.
!*
      INTEGER(kind=i4) I, K, MAXRUL, RULE
      PARAMETER ( MAXRUL = 50 )
      REAL(kind=dp) POINT(MAXRUL), WEIGHT(MAXRUL)
      SAVE RULE
      REAL(kind=dp) W(25,50), T(25,50)
      SAVE W, T
#if defined(_OPENMP)
      !$OMP THREADPRIVATE(W,T)
#endif
