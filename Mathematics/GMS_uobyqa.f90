...
SUBROUTINE uobyqa(n, x, rhobeg, rhoend, iprint, maxfun)
      !dir$ attributes code_align : 32 :: uobyqa
      !dir$ optimize : 32
...
SUBROUTINE uobyqb(n, x, rhobeg, rhoend, iprint, maxfun, npt)
      !dir$ attributes code_align : 32 :: uobyqb
      !dir$ optimize : 3
      !dir$ attributes optimization_parameter:TARGET_ARCH=skylake_avx512 :: uobyqb
      use omp_lib
INTEGER(i4), INTENT(IN)        :: n
REAL (dp), INTENT(IN OUT)  :: x(:)
REAL (dp), INTENT(IN)      :: rhobeg
...
!dir$ attribute align : 64 :: xbase
!dir$ attribute align : 64 :: xopt
!dir$ attribute align : 64 :: xnew
!dir$ attribute align : 64 :: xpt
!dir$ attribute align : 64 :: pq
!dir$ attribute align : 64 :: pl
!dir$ attribute align : 64 :: h
!dir$ attribute align : 64 :: g
!dir$ attribute align : 64 :: d
!dir$ attribute align : 64 :: vlag
...
INTEGER(i4)    :: i, ih, ip, iq, iw, j, jswitch, k, knew, kopt, ksave, ktemp
INTEGER(i4)    :: nf, nftest, nnp, nptm
...
DO  j = 1, n
  ...
  !dir$ assume_aligned pq:64
  !dir$ assume_aligned xopt:64
  !$omp simd simdlen(8) reduction(+:g)
  DO  i = 1, j
    ih = ih + 1
...
DO  j = 1, n
  ...
  !dir$ assume_aligned xopt:64
  !dir$ assume_aligned pq:64
  !$omp simd simdlen(8) reduction(+:vquad)
  DO  i = 1, j
    ih = ih + 1
...
290 DO  k = 1, npt
  ...
  !dir$ assume_aligned xpt:64
  !dir$ assume_aligned xopt:64
  !$omp simd simdlen(8) reduction(+:w)
  DO  i = 1, n
...
IF (knew > 0) THEN
  ...
  DO  j = 1, n
    ...
    !dir$ assume_aligned g:64
    !dir$ assume_aligned xopt:64
    !$omp simd simdlen(8) reduction(+:sumh) reduction(+:g)
    DO  i = 1, j
      ih = ih + 1
      temp = pl(knew,ih)
      g(j) = g(j) + temp * xopt(i)
...
!     function in the trust region.
  ...
  IF (errtol > zero) THEN
    ...
    sumg = zero
    !dir$ assume_aligned g:64
    !$omp simd simdlen(8) reduction(+:sumg)
    DO  i = 1, n
      sumg = sumg + g(i) ** 2
    END DO
    estim = rho * (SQRT(sumg)+rho*SQRT(half*sumh))
    wmult = sixthm * distest ** 1.5_dp
  ...
  DO  j = 1, n
    ...
    DO  i = 1, j
      ...
      IF (i < j) THEN
        ...
        !dir$ assume_aligned pl:64
        !dir$ assume_aligned xopt:64
        !$omp simd simdlen(8) reduction(+:pl)
        DO  k = 1, npt
...
SUBROUTINE trstep(n, g, h, delta, tol, d, evalue)
      !dir$ attributes code_align : 32 :: trstep
      !dir$ optimize : 3
      !dir$ attributes optimization_parameter:TARGET_ARCH=skylake_avx512 :: trstep
      use omp_lib
INTEGER(i4), INTENT(IN)        :: n
REAL (dp), INTENT(IN)      :: g(:)
...
!dir$ attribute align : 64 :: gg
!dir$ attribute align : 64 :: td
!dir$ attribute align : 64 :: tn
!dir$ attribute align : 64 :: w
!dir$ attribute align : 64 :: piv
...
INTEGER(i4)    :: i, iterc, j, jp, k, kp, kpp, ksav, ksave, nm
...
DO  k = 1, nm
  ...
  ELSE
    ...
    DO  j = kp, n
      ...
      IF (j < n) THEN
        jp = j + 1
        !dir$ assume_aligned h:64,w:64,z:64
        !$omp simd simdlen(8) reduction(+:h)
        DO  i = jp, n
...
DO  k = 1, nm
  ...
  END DO
   !dir$ assume_aligned gg:64
   !dir$ assume_aligned h:64
   !$omp simd simdlen(8) reduction(+:gg)
  DO  i = kp, n
    gg(i) = gg(i) - sum * h(i,k)
  END DO
END DO
...
250 w(1) = -gg(1) / piv(1)
!dir$ assume_aligned gg:64,w:64,piv:64,tn:64
!$omp simd simdlen(8)
DO  i = 2, n
  w(i) = (-gg(i)-tn(i-1)*w(i-1)) / piv(i)
END DO
d(n) = w(n)
!dir$ assume_aligned d:64,w:64,tn:64,piv:64
!$omp simd simdlen(8)
DO  i = nm, 1, -1
...
dsq = zero
wsq = zero
!dir$ assume_aligned d:64,w:64,piv:64
!$omp simd simdlen(8) reduction(+:dsq) reduction(+:wsq)
DO  i = 1, n
  dsq = dsq + d(i) ** 2
  wsq = wsq + piv(i) * w(i) ** 2
END DO
...
!     If required, calculate Z for the alternative test for convergence.
  ...
  IF (posdef == zero) THEN
    w(1) = one / piv(1)
    !dir$ assume_aligned tn:64,w:64,piv:64
    ...
    zsq = zero
    dtz = zero
    !dir$ assume_aligned piv:64,w:64,z:64,z:64
    !$omp simd simdlen(8) reduction(+:wwsq) reduction(zsq:64) reduction(+:dtz)
    DO  i = 1, n
...
!     Apply the alternative test for convergence.
    ...
    IF (temp >= zero) THEN
      !dir$ assume_aligned d:64,z:64
      !$omp simd simdlen(8) reduction(+:z)
      DO  i = 1, n
        d(i) = d(i) + gam * z(i)
      END DO
      GO TO 380
...
DO  k = nm, 1, -1
  kp = k + 1
  sum = zero
  !dir$ assume_aligned d:64,h:64
  !$omp simd simdlen(8) reduction(+:sum)
  DO  i = kp, n
    sum = sum + d(i) * h(i,k)
  END DO
  !dir$ assume_aligned d:64,h:64
  !$omp simd simdlen(8) reduction(-:d)
  DO  i = kp, n
    d(i) = d(i) - sum * h(i,k)
  END DO
END DO
...
SUBROUTINE lagmax(n, g, h, rho, d, v, vmax)
      !dir$ attributes code_align : 32 :: lagmax
      !dir$ optimize : 3
      !dir$ attributes optimization_parameter:TARGET_ARCH=skylake_avx512 :: lagmax
INTEGER(i4), INTENT(IN)     :: n
REAL (dp), INTENT(IN)   :: g(:)
...
vhv = zero
dsq = zero
!dir$ assume_aligned v:64,h:64,v:64,d:64
!$omp simd simdlen(8) reduction(+:vsq) reduction(+:vhv) reduction(+:dsq)
DO  i = 1, n
  vsq = vsq + v(i) ** 2
...
IF (vhv*vhv <= 0.9999D0*dsq*vsq) THEN
  temp = vhv / vsq
  wsq = zero
  !dir$ assume_aligned d:64,v:64
  !$omp simd simdlen(8) reduction(-:d) reduction(+:wsq)
  DO  i = 1, n
    d(i) = d(i) - temp * v(i)
    wsq = wsq + d(i) ** 2
  END DO
  whw = zero
  ratio = SQRT(wsq/vsq)
  !dir$ assume_aligned h:64,d:64,v:64
  !$omp simd simdlen(8) reduction(+:whw) reduction(+:v) private(temp)
  DO  i = 1, n
...
dd = zero
dhd = zero
!dir$ assume_aligned g:64,d:64,h:64
!$omp simd simdlen(8) reduction(+:gg) reduction(+:gd) reduction(+:dd) reduction(+:dhd)
DO  i = 1, n
  gg = gg + g(i) ** 2
  gd = gd + g(i) * d(i)
  dd = dd + d(i) ** 2
...
!dir$ assume_aligned v:64,d:64
!$omp simd simdlen(8) reduction(*:d) reduction(+:vv)
DO  i = 1, n
  v(i) = d(i) - temp * g(i)
  vv = vv + v(i) ** 2
  d(i) = scale * d(i)
...
vhg = zero
vhv = zero
!dir$ assume_aligned g:64,v:64,h:64
!$omp simd simdlen(8) reduction(+:ghg) reduction(+:vhg) reduction(+:vhv) 
DO  i = 1, n
...
tempc = wcos / vnorm
tempd = wsin / gnorm
!dir$ assume_aligned d:64,v:64,g:64
!$omp simd simdlen(8)
DO  i = 1, n
...
END IF
!dir$ assume_aligned d:64,d:64,v:64
!$omp simd simdlen(8) 
DO  i = 1, n
