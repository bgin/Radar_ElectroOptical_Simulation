...
#include "GMS_scalv_avx_unrolled8x.h"
	  ...
	  if(__builtin_expect(0.0f==alpha,0)) {
	   ...
	   if(__builtin_expect(1==incx,1)) {
        ...
        void
        sscalv_a_ymm8r4_unroll8x(const int32_t n,
                                const float alpha,
...
#pragma code_align(32)
	 ...
	 void
         sscalv_a_ymm8r4_unroll8x_omp(const int32_t n,
                                      const float alpha,
			              float * __restrict __ATTR_ALIGN__(32) x,
			              const int32_t incx) {
...
#pragma omp parallel for schedule(static,64) default(none) private(i)  \
	 ...
	 void
         dscalv_u_ymm4r8_unroll8x(const int32_t n,
                                  const double alpha,
                                  double * __restrict x,
                                  const int32_t incx) {
                 if(__builtin_expect(0==n,0) ||
	 ...
	 void
         dscalv_a_ymm4r8_unroll8x(const int32_t n,
                                  const double alpha,
...
#pragma code_align(32)
	  ...
	  void
          dscalv_a_ymm4r8_unroll8x_omp(const int32_t n,
                                       const double alpha,
                                       double * __restrict __ATTR_ALIGN__(32) x,
                                       const int32_t incx) {
