...
module GMS_kmeans  
 ...
 !                         Date: 13-05-2018
  !                        Time: 10:22 GMT+2
 !          Modified:      09-11-2020, 04:32PM GMT+2    
 !
 !          Version:
 !
 !                      Major: 1
 ...
 !==================================================================================85
    ...
    use mod_kinds, only : int4, int8b, sp, dp
...
contains
  ...
  subroutine ch_cap ( c )  !GCC$ ATTRIBUTES cold :: ch_cap !GCC$ ATTRIBUTES aligned(32) :: ch_cap !GCC$ ATTRIBUTES inline :: ch_cap
#elif defined __INTEL_COMPILER
  subroutine ch_cap(c)
    !DIR$ ATTRIBUTE CODE_ALIGN : 32 :: ch_cap
    !DIR$ ATTRIBUTES INLINE :: ch_cap
#endif
!*****************************************************************************80
!   subroutine ch_cap(c)
...
#if defined __GFORTRAN__ && !defined __INTEL_COMPILER
  function ch_eqi ( c1, c2 )  !GCC$ ATTRIBUTES cold :: ch_eqi !GCC$ ATTRIBUTES aligned(32) :: ch_eqi !GCC$ ATTRIBUTES inline :: ch_eqi
#elif defined __INTEL_COMPILER
  function ch_eqi(c1,c2)
    !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: ch_eqi 
    !DIR$ ATTRIBUTES INLINE :: ch_eqi
!*****************************************************************************80
!
...
#if defined __GFORTRAN__ && !defined __INTEL_COMPILER
   subroutine ch_to_digit ( c, digit )  !GCC$ ATTRIBUTES cold :: ch_to_digit !GCC$ ATTRIBUTES aligned(32) :: ch_to_digit !GCC$ ATTRIBUTES inline :: ch_to_digit
#elif defined __INTEL_COMPILER
   subroutine ch_to_digit(c,digit)
      !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: ch_to_digit
      !DIR$ ATTRIBUTES INLINE :: ch_to_digit
#endif
!*****************************************************************************80
!
...
cluster, cluster_center, cluster_energy ) !GCC$ ATTRIBUTES hot :: cluster_energy_compute !GCC$ ATTRIBUTES aligned(32) :: cluster_energy_compute !GCC$ ATTRIBUTES inline :: cluster_energy_compute
#elif defined __INTEL_COMPILER
...
!
  ...
  integer ( kind = int4 ) cluster_num
  integer ( kind = int4 ) dim_num
  integer ( kind = int4 ) point_num

  integer ( kind = int4 ), dimension ( point_num ) :: cluster
  real ( kind = dp ), dimension ( dim_num, cluster_num ) :: cluster_center
  real ( kind = dp ), dimension ( cluster_num ) :: cluster_energy
  integer ( kind = int4 ) i
  integer ( kind = int4 ) j
  real ( kind = dp ), dimension ( dim_num, point_num ) :: point
  real ( kind = dp ) point_energy

  cluster_energy(1:cluster_num) = 0.0_dp
#if defined __INTEL_COMPILER
  !DIR$ ASSUME_ALIGNED cluster_energy:64
  !DIR$ ASSUME_ALIGNED cluster:64
  !DIR$ ASSUME_ALIGNED cluster_center:64
  !DIR$ ASSUME_ALIGNED point:64
#endif
#if defined __INTEL_COMPILER
  !DIR$ VECTOR ALIGNED
  !DIR$ VECTOR ALWAYS
  ...
  do i = 1, point_num
    ...
    point_energy = sum ( &
      point(1:dim_num,i)-cluster_center(1:dim_num,j)*point(1:dim_num,i)-cluster_center(1:dim_num,j))
...
end subroutine cluster_energy_compute

#if defined __GFORTRAN__ && !defined __INTEL_COMPILER
subroutine cluster_initialize_1 ( dim_num, point_num, cluster_num, point, &
  cluster_center )  !GCC$ ATTRIBUTES hot :: cluster_initialize_1 !GCC$ ATTRIBUTES aligned(32) :: cluster_initialize_1 !GCC$ ATTRIBUTES inline :: cluster_initialize_1
#elif defined __INTEL_COMPILER
subroutine cluster_initialize_1 ( dim_num, point_num, cluster_num, point, &
     cluster_center )
      !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: cluster_initialize_1
      !DIR$ ATTRIBUTES INLINE :: cluster_initialize_1
#if defined __AVX512F__
      !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: "target_arch=AVX512" :: cluster_initialize_1
#else
      !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: "target_arch=AVX" :: cluster_initialize_1
#endif
#endif
!*****************************************************************************80
!
...
!
  ...
  real ( kind = dp ) cluster_center(dim_num,cluster_num)
  integer ( kind = int4 ) i
  real ( kind = dp ) point(dim_num,point_num)
#if defined __INTEL_COMPILER
  !DIR$ ASSUME_ALIGNED cluster_center:64
  !DIR$ ASSUME_ALIGNED point:64
#endif
  ...
  return
end subroutine cluster_initialize_1

#if defined __GFORTRAN__  && !defined __INTEL_COMPILER
subroutine cluster_initialize_2 ( dim_num, point_num, cluster_num, point, &
     seed, cluster_center ) !GCC$ ATTRIBUTES hot :: cluster_initialize_2 !GCC$ ATTRIBUTES aligned(32) :: cluster_initialize_2 !GCC$ ATTRIBUTES inline :: cluster_initialize_2
#elif defined __INTEL_COMPILER
subroutine cluster_initialize_2 ( dim_num, point_num, cluster_num, point, &
     seed, cluster_center )
      !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: cluster_initialize_1
      !DIR$ ATTRIBUTES INLINE :: cluster_initialize_1
#if defined __AVX512F__
      !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: "target_arch=AVX512" :: cluster_initialize_2
#else
      !DIR$ ATTRIBUTES OPTIMIZATION_PARAMETER: "target_arch=AVX" :: cluster_initialize_2
#endif
#endif
!*****************************************************************************80
!
...
!
  ...
  real ( kind = dp ) r_max(dim_num)
  !DIR$ ATTRIBUTES ALIGN : r_max
  real ( kind = dp ) r_min(dim_num)
  !DIR$ ATTRIBUTES ALIGN : r_min
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
  real ( kind = dp ) r(dim_num)     !GCC$ ATTRIBUTES aligned(64) :: r
  real ( kind = dp ) r_max(dim_num) !GCC$ ATTRIBUTES aligned(64) :: r_max
  real ( kind = dp ) r_min(dim_num) !GCC$ ATTRIBUTES aligned(64) :: r_min
#endif
  integer ( kind = int4 ) seed
  ...
  do i = 1, cluster_num
    ...
    cluster_center(1:dim_num,i) = &
      ( 1.0_dp - r(1:dim_num) ) * r_min(1:dim_num) &
                + r(1:dim_num)   * r_max(1:dim_num)
  end do
    
  
end subroutine cluster_initialize_2
...
!
  ...
  real ( kind = dp ) cluster_center(dim_num,cluster_num)
  integer ( kind = int4 ) cluster_population(cluster_num)
  integer ( kind = int4 ) i
  integer ( kind = int4 ) i4_uniform
  integer ( kind = int4 ) j
  real ( kind = dp ) point(dim_num,point_num)
  integer ( kind = int4 ) seed
!
...
!
  ...
  real ( kind = dp ) cluster_center(dim_num,cluster_num)
  real ( kind = dp ) cluster_factor(cluster_num)
  real ( kind = dp ) cluster_weight(cluster_num)
  real ( kind = dp ) divisor
  integer ( kind = int4 ) i
  integer ( kind = int4 ) j
  real ( kind = dp ) point(dim_num,point_num)
  integer ( kind = int4 ) seed

  cluster_center(1:dim_num,1:cluster_num) = 0.0e+00_dp
  cluster_weight(1:cluster_num) = 0.0e+00_dp
...
!
  ...
  real ( kind = dp ) cluster_center(dim_num,cluster_num)
  real ( kind = dp ) column_sum
  real ( kind = dp ) factor(point_num,cluster_num)
  integer ( kind = int4 ) j
  real ( kind = dp ) point(dim_num,point_num)
  integer ( kind = int4 ) seed
!
...
!
  ...
  real ( kind = dp ) ce
  integer ( kind = int4 ) cep
  real ( kind = dp ) ce_total
  real ( kind = dp ) cluster_energy(cluster_num)
  integer ( kind = int4 ) cluster_population(cluster_num)
  real ( kind = dp ) cluster_variance(cluster_num)
  integer ( kind = int4 ) cp
  integer ( kind = int4 ) cpp
  real ( kind = dp ) cv
  integer ( kind = int4 ) i
  integer ( kind = int4 ) point_num
  ...
  do i = 1, cluster_num
    ...
    cpp = int ( real ( 100 * cp, kind = dp ) / real ( point_num, kind = dp ) )
    ce = cluster_energy(i)
    cep = int ( ( ce * 100.0E+00_dp ) / ce_total )
...
!
  ...
  integer ( kind = int4 ) cluster(point_num)
  real ( kind = dp ) cluster_center(dim_num,cluster_num)
  integer ( kind = int4 ) cluster_population(cluster_num)
  real ( kind = dp ) cluster_variance(cluster_num)
  integer ( kind = int4 ) i
  integer ( kind = int4 ) j
  real ( kind = dp ) point(dim_num,point_num)
  real ( kind = dp ) point_variance

  cluster_population(1:cluster_num) = 0
  cluster_variance(1:cluster_num) = 0.0E+00_dp
...
!
  ...
  integer   ( kind = int4 )   column_num
  logical                  got_one
  character ( len = * )    input_filename
  integer   ( kind = int4 )   input_status
  integer   ( kind = int4 )   input_unit
...
!
  ...
  integer   ( kind = int4 )   bad_num
  integer   ( kind = int4 )   comment_num
  integer   ( kind = int4 )   ierror
  character ( len = * )    input_filename
  integer   ( kind = int4 )   input_status
  integer   ( kind = int4 )   input_unit
  character ( len = 255 )  line
  integer   ( kind = int4 )   record_num
  integer   ( kind = int4 )   row_num
...
!
  ...
  integer ( kind = int4 ) c
  real ( kind = dp ), dimension ( dim_num, cluster_num ) :: centroid
  integer ( kind = int4 ), dimension ( point_num ) :: cluster
  real ( kind = dp ), dimension ( dim_num, cluster_num ) :: cluster_center
  real ( kind = dp ), dimension ( cluster_num ) :: cluster_energy
  integer ( kind = int4 ), dimension ( cluster_num ) :: cluster_population
  logical, parameter :: debug = .true.
  integer ( kind = int4 )  i
  integer ( kind = int4 ) it_max
  integer ( kind = int4 ) it_num
  integer ( kind = int4 ) j
  integer ( kind = int4 ) missed
  real ( kind = dp ), dimension ( dim_num, point_num ) :: point
  real ( kind = dp ) point_energy
  real ( kind = dp ) point_energy_min
  integer ( kind = int4 ) swap
!
!  Data checks.
!
...
!
    centroid(1:dim_num,1:cluster_num) = 0.0E+00_dp
    cluster_population(1:cluster_num) = 0
    ...
    do j = 1, cluster_num
      ...
      if ( cluster_population(j) /= 0 ) then
        centroid(1:dim_num,j) = centroid(1:dim_num,j) &
          / real ( cluster_population(j), kind = dp)
      else
        missed = missed + 1
...
!
    cluster_energy(1:cluster_num) = 0.0E+00_dp
...
!
  ...
  real ( kind = dp ) energy(cluster_num)
  integer ( kind = int4 ) i
  integer ( kind = int4 ) i4_uniform
  integer ( kind = int4 ) it_max
  integer ( kind = int4 ) it_num
  integer ( kind = int4 ) j
  integer ( kind = int4 ) list(1)
  real ( kind = dp ) point(dim_num,point_num)
  real ( kind = dp ) point_energy
  real ( kind = dp ) point_energy_min
  integer ( kind = int4 ) seed
...
!
    ...
    do i = 1, cluster_num
      if ( cluster_population(i) /= 0 ) then
        cluster_center(1:dim_num,i) = cluster_center(1:dim_num,i) / &
          real ( cluster_population(i), kind = dp)
      else
...
!
  ...
  integer ( kind = int4 ) c
  real ( kind = dp ), dimension ( dim_num, cluster_num ) :: centroid
  integer ( kind = int44 ), dimension ( point_num ) :: cluster
  real ( kind = dp ), dimension ( dim_num, cluster_num ) :: cluster_center
  real ( kind = dp ), dimension ( cluster_num ) :: cluster_energy
  integer ( kind = int4 ), dimension ( cluster_num ) :: cluster_population
  real ( kind = dp ), dimension ( cluster_num ) :: cluster_weight
  logical, parameter :: debug = .true.
  real ( kind = dp ) energy
  integer ( kind = int4 ) i
  integer ( kind = int4 ) it_max
...
!
  ...
  real ( kind = dp ) dc
  real ( kind = dp) de
  real ( kind = dp ) f(point_num)
  integer ( kind = int4 ) i
  integer ( kind = int4 ) il
...
!
  ...
  integer ( kind = int4 ) cluster_num
  integer ( kind = int44 ) dim_num
  integer ( kind = int4 ) point_num

  real ( kind = dp ) an1(cluster_num)
  real ( kind = dp ) an2(cluster_num)
  integer ( kind = int4 ) cluster(point_num)
  integer ( kind = int44 ) cluster2(point_num)
  real ( kind = dp ) cluster_center(dim_num,cluster_num)
  real ( kind = dp ) cluster_energy(cluster_num)
  integer ( kind = int4 ) cluster_population(cluster_num)
  real ( kind = dp ) d(point_num)
  real ( kind = dp ) db
  real ( kind = dp ) dt(2)
  integer ( kind = int4 ) i 
  integer ( kind = int4 ) ifault
  integer ( kind = int4 ) il
  integer ( kind = int4 ) indx
  integer ( kind = int4 ) it_max
  integer ( kind = int4 ) it_num
  integer ( kind = int4 ) itran(cluster_num)
  integer ( kind = int4 ) j
  integer ( kind = int4 ) l
  integer ( kind = int4 ) live(cluster_num)
  integer ( kind = int4 ) ncp(cluster_num)
  real ( kind = dp ) point(dim_num,point_num)
  real ( kind = dp ) temp
...
!
  ...
  do j = 1, dim_num
    cluster_center(j,1:cluster_num) = cluster_center(j,1:cluster_num) &
      / real ( cluster_population(1:cluster_num), kind = dp )
  end do
!
!  Compute the cluster energies.
!
...
!
  ...
  real ( kind = dp ) an2(cluster_num)
  integer ( kind = int4 ) cluster(point_num)
  integer ( kind = int4 ) cluster2(point_num)
  real ( kind = dp ) cluster_center(dim_num,cluster_num)
  integer ( kind = int4 ) cluster_population(cluster_num)
  real ( kind = dp ) d(point_num)
  real ( kind = dp ) dc
  ...
  integer ( kind = int4 ) l2
  integer ( kind = int4 ) live(cluster_num)
  integer ( kind = int4 ) ll
  integer ( kind = int4 ) ncp(cluster_num)
  real ( kind = dp ) point(dim_num,point_num)
  real ( kind = dp ) r2
  real ( kind = dp ) rr
...
!
  ...
  integer ( kind = int4 ) ci
  integer ( kind = int4 ) cj
  integer ( kind = int4 ), dimension ( point_num ) :: cluster
  real ( kind = dp ), dimension ( dim_num, cluster_num ) :: cluster_center
  real ( kind = dp ), dimension ( cluster_num ) :: cluster_energy
  integer ( kind = int4 ), dimension ( cluster_num ) :: cluster_population
  logical, parameter :: debug = .true.
  real ( kind = dp ), dimension ( cluster_num ) :: distsq
  integer ( kind = int4 ) i
  integer ( kind = int4 ) it_max
  integer ( kind = int4 ) it_num
  integer ( kind = int4 ) j
      ...
      do cj = 1, cluster_num
        ...
        if ( cj == ci ) then
          ...
          distsq(cj) = sum ( &
            ...
            * real ( cluster_population(cj), kind = dp) &
            / real ( cluster_population(cj) - 1, kind = dp )

        else if ( cluster_population(cj) == 0 ) then

          cluster_center(1:dim_num,cj) = point(1:dim_num,i)
          distsq(cj) = 0.0e+00_dp
...
!
      ...
      cluster_center(1:dim_num,ci) = &
        ( real ( cluster_population(ci), kind = dp) &
        * cluster_center(1:dim_num,ci) &
        - point(1:dim_num,i) ) / real ( cluster_population(ci) - 1, kind = dp)

      cluster_center(1:dim_num,cj) = &
        ( real ( cluster_population(cj), kind = dp) &
        * cluster_center(1:dim_num,cj) &
        + point(1:dim_num,i) ) / real ( cluster_population(cj) + 1, kind = dp)
...
!
  ...
  !DIR$ VECTOR ALIGNED
  do i = 1, cluster_num
    if ( 0.0e+00 < cluster_weight(i) ) then
      cluster_center(1:dim_num,i) = cluster_center(1:dim_num,i) &
        / cluster_weight(i)
    end if
  end do
...
!
  ...
  !DIR$ VECTOR ALIGNED
  do i = 1, cluster_num
    if ( 0.0E+00_dp < cluster_weight(i) ) then
      cluster_center(1:dim_num,i) = cluster_center(1:dim_num,i) &
        / cluster_weight(i)
    end if
  end do
...
!
  ...
  r4_uniform_01 = real ( seed, kind = 4 ) * 4.656612875E-10_sp
...
!
  r8_uniform_01 = real ( seed, kind = 8) * 4.656612875E-10_dp
...
!
  ...
  do j = 1, n
    ...
    do i = 1, m
      ...
      r(i,j) = real ( seed, kind = 8 ) * 4.656612875E-10_dp
...
!
  ...
  integer   ( kind = int4 ) j
  character ( len = * )  output_filename
  integer   ( kind = int4 ) output_status
  integer   ( kind = int4 ) output_unit
  character ( len = 30 ) string
  real      ( kind = dp ) table(m,n)
!
!  Open the file.
...
!
  ...
  do i = 1, n
    ...
    r(i) = real ( seed, kind = 8 ) * 4.656612875e-10_dp
...
!
  ...
  integer ( kind = int4 ) count
  integer ( kind = int4 ) count_max
  integer ( kind = int4 ) count_rate
  logical, parameter :: debug = .false.
  integer ( kind = int4 ) i
  integer ( kind = int4 ) seed
  integer ( kind = int4 ), allocatable :: seed_vector(:)
  integer ( kind = int4 ) seed_size
  real ( kind = dp ) t
!
...
!
  ...
  character c
  integer ( kind = int4 ) ierror
  integer ( kind = int4 ) ihave
  integer ( kind = int4 ) isgn
  integer ( kind = int4 ) iterm
  integer ( kind = int4 ) jbot
  integer ( kind = int4 ) jsgn
  integer ( kind = int4 ) jtop
  integer ( kind = int4 ) lchar
  integer ( kind = int4 ) nchar
  integer ( kind = int4 ) ndig
  real ( kind = dp ) r
  real ( kind = dp ) rbot
  real ( kind = dp ) rexp
  real ( kind = dp ) rtop
...
!
  ...
  logical blank
  integer ( kind = int4 ) i
  integer ( kind = int4 ) lens
  integer ( kind = int4 ) nword
  character ( len = * ) s
