...
module mod_necgw
 ...
 !=================================================================================
    ...
    ! Module full version
    integer(kind=int4), parameter, public :: MOD_NECGW_FULLVER = 1000_int4*MOD_NECGW_MAJOR+100_int4*MOD_NECGW_MINOR + &
...
9999      call handle_fatal_memory_error( iounit, logging,verbose,append,fname,                                                    &
    ...
    subroutine readNECGWires(wf,iounit,filename,errmsg,ioerr,iounit2,logging,verbose,append,fname)
...
9999      close(UNIT=iounit, STATUS='KEEP')
    ...
    subroutine copyNECGWires_ymm8i4_ymm4r8(wf,     &
                                           ymm8i4_ordinal,    &
                                           ymm8i4_nsegs,      &
                                           ymm4r8_startx,     &
                                           ymm4r8_stopx,      &
          ...
          ! Exec code ....
          errstate = .false.
          is_conforming = (size(wf.m_ordinal) == 8*size(ymm8i4_ordinal)) .and.  &
                          (size(wf.m_nsegs)   == 8*size(ymm8i4_nsegs))   .and.  &
                          (size(wf.m_startx)  == 4*size(ymm4r8_startx))  .and.  &
                          (size(wf.m_stopx)   == 4*size(ymm4r8_stopx))   .and.  &
                          (size(wf.m_starty)  == 4*size(ymm4r8_starty))  .and.  &
                          (size(wf.m_stopy)   == 4*size(ymm4r8_stopy))   .and.  &
                          (size(wf.m_startz)  == 4*size(ymm4r8_stopz))   .and.  &
                          (size(wf.m_radii)   == 4*size(ymm4r8_radii))
          ...
          call copy_i4_ymm8i4(ymm8i4_ordinal, wf.m_ordinal)
          call copy_i4_ymm8i4(ymm8i4_nsegs,   wf.m_nsegs)
          call copy_r8_ymm4r8(ymm4r8_startx,  wf.m_startx)
          call copy_r8_ymm4r8(ymm4r8_stopx,   wf.m_stopx)
          call copy_r8_ymm4r8(ymm4r8_starty,  wf.m_starty)
          call copy_r8_ymm4r8(ymm4r8_stopy,   wf.m_stopy)
          call copy_r8_ymm4r8(ymm4r8_startz,  wf.m_startz)
          call copy_r8_ymm4r8(ymm4r8_stopz,   wf.m_stopz)
          call copy_r8_ymm4r8(ymm4r8_radii,   wf.m_radii)
