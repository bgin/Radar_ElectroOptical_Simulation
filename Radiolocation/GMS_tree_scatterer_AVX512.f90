...
!DIR$ ATTRIBUTES ALIGN : 64 :: leaves_incident_angle
        type(ZMM16r4_t),     allocatable, dimension(:,:)   :: leaves_incident_angle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t),     allocatable, dimension(:,:)   :: leaves_incident_angle !GCC$ ATTRIBUTES aligned(64) :: leaves_incident_angle
#endif
...
!DIR$ ATTRIBUTES ALIGN : 64 :: branches_incident_angle
        type(ZMM16r4_t),      allocatable, dimension(:,:)   :: branches_incident_angle
#elif defined __GFORTRAN__  && !defined __INTEL_COMPILER
        type(ZMM16r4_t),      allocatable, dimension(:,:)   :: branches_incident_angle  !GCC$ ATTRIBUTES aligned(64) :: branches_incident_angle
#endif
...
!DIR$ ATTRIBUTES ALIGN : 64 :: leaves_xangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_xangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_xangle  !GCC$ ATTRIBUTES aligned(64) :: leaves_xangle
#endif
...
!DIR$ ATTRIBUTES ALIGN : 64 :: leaves_sin_xangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_sin_xangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_sin_xangle !GCC$ ATTRIBUTES aligned(64) :: leaves_sin_xangle
#endif
        ! Leaves sine of vibration angle in x-axis per PRF/s
        ! 1st dimension PRF/s, 2nd dimension sine of vibrational angle (rad)
#if defined  __ICC || defined __INTEL_COMPILER
!DIR$ ATTRIBUTES ALIGN : 64 :: leaves_cos_xangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_cos_xangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_cos_xangle !GCC$ ATTRIBUTES aligned(64) :: leaves_cos_xangle
#endif
...
!DIR$ ATTRIBUTES ALIGN : 64 :: leaves_yangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_yangle
#elif defined  __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_yangle  !GCC$ ATTRIBUTES aligned(64) :: leaves_yangle
#endif
...
!DIR$ ATTRIBUTES ALIGN : 64 :: leaves_sin_yangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_sin_yangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_sin_yangle  !GCC$ ATTRIBUTES aligned(64) :: leaves_sin_yangle
#endif       
        ! Leaves sine of vibration angle in y-axis per PRF/s
        ! 1st dimension PRF/s, 2nd dimension sine of vibrational angle (rad)
#if defined  __ICC || defined  __INTEL_COMPILER
!DIR$ ATTRIBUTES ALIGN : 64 :: leaves_cos_angle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_cos_yangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: leaves_cos_yangle  !GCC$ ATTRIBUTES aligned(64) :: leaves_cos_yangle
#endif
        ! Branches angle of vibration in x-axis per PRF/s
        ! 1st dimension PRF/s, 2nd dimension angle values (rad)
#if defined  __ICC || defined __INTEL_COMPILER
!DIR$ ATTRIBUTES ALIGN ; 64 :: branches_xangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_xangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_xangle  !GCC$ ATTRIBUTES aligned(64) :: branches_xangle
#endif
        ! Branches sine of vibration angle in x-axis per PRF/s
        ! 1st dimension PRF/s, 2nd dimension angle  of vibrational angle (rad)
#if defined  __ICC || defined __INTEL_COMPILER
!DIR$ ATTRIBUTES ALIGN : 64 :: branches_sin_xangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_sin_xangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_sin_xangle !GCC$ ATTRIBUTES aligned(64) :: branches_sin_xangle
#endif
...
!DIR$ ATTRIBUTES ALIGN : 64 :: branches_cos_xangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_cos_xangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_cos_xangle !GCC$ ATTRIBUTES aligned(64) :: branches_cos_xangle
#endif
        ! Branches angle of vibration in y-axis per PRF/s
        ! 1st dimension PRF/s, 2nd dimension angle values (rad)
#if defined  __ICC || defined __INTEL_COMPILER
!DIR$ ATTRIBUTES ALIGN : 64 :: branches_yangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_yangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_yangle  !GCC$ ATTRIBUTES aligned(64) :: branches_yangle
#endif
        ! Branches sine of vibration angle in y-axis per PRF/s
        ! 1st dimension PRF/s, 2nd dimension sine  of vibrational angle (rad)
#if defined  __ICC || defined __INTEL_COMPILER
!DIR$ ATTRIBUTES ALIGN : 64 :: branches_sin_yangle
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_sin_yangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_sin_yangle  !GCC$ ATTRIBUTES aligned(64) :: branches_sin_yangle
#endif
        ! Branches cosine of vibration angle in x-axis per PRF/s
        ! 1st dimension PRF/s, 2nd dimension cosine  of vibrational angle (rad)
#if defined  __ICC || defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_cos_yangle
#elif defined __GFORTRAN__ && !defined __INTEL_COMPILER
        type(ZMM16r4_t), allocatable, dimension(:,:)  :: branches_cos_yangle  !GCC$ ATTRIBUTES aligned(64) :: branches_cos_yangle
#endif
   ...
   contains
     ...
     subroutine InitTreeScatterer_AVX512(DataCold,DataHot,nleaves,nbranches,nsteps,ordinal,trunk_param_npoints, &
     ...
     subroutine InitTreeScatterer_AVX512(DataCold,DataHot,nleaves,nbranches,nsteps,ordinal,trunk_param_npoints, &
       ...
       !DIR$ ATTRIBUTES CODE_ALIGN : 32 :: InitTreeScatterer_AVX512
           ...
           allocate(DataCold.leaves_moistness(DataCold.nleaves), &
                    ...
                    DataCold.leaves_incident_angle(DataCold.nleaves,2), &
                    ...
                    DataCold.branches_incident_angle(DataCold.nbranches,2), &
                    ...
                    DataHot.leaves_xangle(DataCold.nsteps,DataCold.nleaves), &
                    DataHot.leaves_sin_xangle(DataCold.nsteps,DatacOLD.nleaves), &
                    DataHot.leaves_cos_xangle(DataCold.nsteps,DataCold.nleaves), &
                    DataHot.leaves_yangle(DataCold.nsteps,DataCold.nleaves), &
                    DataHot.leaves_sin_yangle(DataCold.nsteps,DataCold.nleaves), &
                    DataHot.leaves_cos_yangle(DataCold.nsteps,DataCold.nleaves), &
                    DataHot.branches_xangle(DataCold.nsteps,DataCold.nbranches), &
                    DataHot.branches_sin_xangle(DataCold.nsteps,DataCold.nbranches), &
                    DataHot.branches_cos_xangle(DataCold.nsteps,DataCold.nbranches), &
                    DataHot.branches_yangle(DataCold.nsteps,DataCold.nbranches), &
                    DataHot.branches_sin_yangle(DataCold.nsteps,DataCold.nbranches), &
                    DataHot.branches_cos_yangle(DataCold.nsteps,DataCold.nbranches), &
                                  STAT=aerr,ERRMSG=emsg)
                    if(aerr /= 0) goto 9999  
                    DataHot.tree_xangle = INITVALR4 ! Initialization still here and will moved to the approrpiate computational routines
                    DataHot.tree_sin_xangle = INITVAL
                    DataHot.tree_cos_xangle = INITVAL
                    DataHot.tree_yangle = INITVALR4
                    DataHot.tree_sin_yangle = INITVAL
                    DataHot.tree_cos_yangle = INITVAL
                    DataHot.tree_cross_section = INITVAL
                    DataHot.crown_cross_section = INITVAL
                    DataHot.trunk_cross_section = INITVAL   
