...
!===================================================================================85
 ...
 !          
 !          Purpose:
 !                      J. Burkardt 'obj_io' implementation wrapped in
 !                      a module.
 !          History:
 !                        
 !                        Date: 01-01-2022
 ...
 !=================================================================================
     ...
     type, public :: OBJ_Mesh
           ...
           integer(kind=i4) :: normal_num        ! NORMAL_NUM, the number of normal vectors.
           integer(kind=i4) :: order_max         ! ORDER_MAX, the maximum number of vertices per face
#if defined(__INTEL_COMPILER) || defined(__ICC)   
           real(kind=dp),    allocatable, dimension(:,:) :: node_xyz   !  NODE_XYZ(3,NODE_NUM), the coordinates of points. the coordinates of points.
           integer(kind=i4), allocatable, dimension(:)   :: face_order !  FACE_ORDER(FACE_NUM), the number of vertices per face
           integer(kind=i4), allocatable, dimension(:,:) :: face_node  !  FACE_NODE(ORDER_MAX,FACE_NUM), the nodes making faces
           real(kind=dp),    allocatable, dimension(:,:) :: normal_vector ! NORMAL_VECTOR(3,NORMAL_NUM), normal vectors.
           integer(kind=i4), allocatable, dimension(:,:) :: vertex_normal ! VERTEX_NORMAL(ORDER_MAX,FACE_NUM), the indices !    of normal vectors per vertex.
           !DIR$ ATTRIBUTES ALIGN : 64 :: node_xyz
           !DIR$ ATTRIBUTES ALIGN : 64 :: face_order
           !DIR$ ATTRIBUTES ALIGN : 64 :: face_node
           !DIR$ ATTRIBUTES ALIGN : 64 :: normal_vector
           !DIR$ ATTRIBUTES ALIGN : 64 :: vertex_normal
#elif defined __GFORTRAN__ && (!defined(__ICC) || !defined(__INTEL_COMPILER))
           real(kind=dp),    allocatable, dimension(:,:) :: node_xyz      !GCC$ ATTRIBUTES aligned(64) :: node_xyz 
           integer(kind=i4), allocatable, dimension(:)   :: face_order    !GCC$ ATTRIBUTES aligned(64) :: face_order 
           integer(kind=i4), allocatable, dimension(:,:) :: face_node     !GCC$ ATTRIBUTES aligned(64) :: face_node
           real(kind=dp),    allocatable, dimension(:,:) :: normal_vector !GCC$ ATTRIBUTES aligned(64) :: normal_vector
           integer(kind=i4), allocatable, dimension(:,:) :: vertex_normal !GCC$ ATTRIBUTES aligned(64) :: vertex_normal
#endif
     end type OBJ_Mesh
...
!
  ...
  integer ( kind = i4 ) face_node(order_max,face_num)
...
!
  ...
  integer ( kind = i4 ) normal_num

  integer ( kind = i4 ) face
  integer ( kind = i4 ) normal
  real ( kind = dp ) normal_vector(3,normal_num)
...
!
  ...
  integer ( kind = i4 ) input_file_unit
  integer ( kind = i4 ) input_file_status
  integer ( kind = i4 ) itemp
  ...
  real ( kind = dp ) temp
  integer ( kind = i4 ) vertex
  integer ( kind = i4 ) vertex_normal(order_max,face_num)
  character ( len = 255 ) word
  integer ( kind = i4 ) word_index
...
!
  ...
  integer ( kind = i4 ) normal
  real ( kind = dp ) normal_vector(3,normal_num)
  integer ( kind = i4 ) output_file_unit
  character ( len = * ) output_filename
  integer ( kind = i4 ) output_file_status
