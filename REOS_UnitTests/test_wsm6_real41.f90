 ...
 ifort -o unit_test_wsm6d_real41 -fp-model fast=2 -ftz -O3 -ggdb  -march=skylake-avx512 -mavx512f \
    ...
    -qopenmp -fpp -falign-functions=32 -qopt-report=5 GMS_config.fpp GMS_kinds.f90  \
     GMS_wsm6_kernel.f90  GMS_wsm6_driver.f90 unit_test_wsm6_real41.f90
    ASM:
    ifort -S unit_test_wsm6d_real41 -fp-model fast=2 -ftz -O3 -ggdb  -march=skylake-avx512 -mavx512f \
    -align array64byte -falign-loops=32 -fno-math-errno -fopenmp -fvec-with-mask -fvec-peel-loops \
    -qopt-assume-safe-padding -qopt-malloc-options=4 -qopt-multiple-gather-scatter-by-shuffles    \
    -qopt-zmm-usage=high -simd -unroll \
    -qopenmp -fpp -falign-functions=32 -qopt-report=5 GMS_config.fpp GMS_kinds.f90  \
...
program main 
    ...
    character(len=60), parameter  :: header = "[UNIT-TEST #1: wsm62D, case: [real41] -- START]"
    character(len=60), parameter  :: footer = "[UNIT-TEST #1: wsm62D, case: [real41] -- END]"
    integer(kind=i4)      :: ids,ide,jds,jde,kds,kde 
    integer(kind=i4)      :: ims,ime,jms,jme,kms,kme 
    integer(kind=i4)      :: its,ite,jts,jte,kts,kte 
    real(kind=sp)         :: start,end,delta 
    print*, header 
    call cpu_time(start)
    ...
    call cpu_time(end)                
    write(6,*) '**Crude Timing!!**, wsm6D_driver executed in:', end-start, 'seconds.'                 
