...
!DIR$ ATTRIBUTES VECTOR :: copy_r4_ymm8r4
          ...
          type(YMM8r4_t),             dimension(:), intent(inout) :: output
          real(kind=sp),  contiguous, dimension(:), intent(in)    :: input
          ! Locals
...
!DIR$ ATTRIBUTES VECTOR :: copy_r4_zmm16r4
    ...
    type(ZMM16r4_t),            dimension(:), intent(inout) :: output
    real(kind=sp),  contiguous, dimension(:), intent(in)    :: input
    ! Locals
    integer(kind=int4) :: i,j
    ! Exec code ....
...
!DIR$  ATTRIBUTES VECTOR :: copy_r8_ymm4c8
    ...
    type(YMM4c8_t),            dimension(:), intent(inout) :: output
...
!DIR$ VECTOR ALWAYS
        do j = 0, 3
            output(i).re(j) = input(i+v4len*j)
            output(i).im(j) = input(i+v4len*j)
        end do
    end do
...
!DIR$ ATTRIBUTES VECTOR :: copy_r8_zmm8c8
    ...
    type(ZMM8c8_t),            dimension(:),   intent(inout) :: output
    real(kind=dp), contiguous, dimension(:), intent(in)      :: input1
    real(kind=dp), contiguous, dimension(:), intent(in)      :: input2
    ! Locals
    integer(kind=int4) :: i,j
